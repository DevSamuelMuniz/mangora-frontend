...
export default function FinancialOverview() {
  ...
  async function confirmPayment(input: PaymentInput) {
    ...
    try {
      ...
      setEntryToPay(null);
      setNotice(t("finance.toasts.paid"));
    } catch (requestError) {
      setActionError(requestError instanceof Error ? requestError.message : t("finance.errors.confirmPayment"));
    }
  }
  ...
  async function confirmReverse(reason: string) {
    ...
    try {
      ...
      setNotice(t("finance.toasts.reversed"));
    } catch (requestError) {
      setActionError(requestError instanceof Error ? requestError.message : t("finance.errors.reverse"));
    }
  }
  ...
  async function runRecurring() {
    try {
      ...
      setNotice(result.generated ? `${result.generated} lançamento(s) recorrente(s) gerado(s).` : t("finance.noneGenerated"));
    } catch (requestError) {
      setActionError(requestError instanceof Error ? requestError.message : t("finance.errors.generate"));
    }
  }
    ...
    <>
      <section>
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">{t("finance.eyebrow")}</p>
            <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">{t("finance.title")}</h1>
            <p className="mt-1 text-xs text-slate-500">{t("finance.subtitle")}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link href="/financeiro?acao=novo" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-sm font-bold text-white shadow-lg shadow-orange-200 transition hover:-translate-y-0.5 hover:shadow-xl"><Plus className="size-4" />{t("finance.actions.create")}</Link>
          </div>
        </div>

        {notice && <div role="status" className="mt-4 flex items-center justify-between rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-xs font-semibold text-green-700"><span>{notice}</span><button type="button" onClick={() => setNotice("")} className="font-bold underline">{t("finance.actions.close")}</button></div>}
        ...
        <div className="mt-4 grid items-stretch gap-4 xl:grid-cols-[1.3fr_0.7fr]">
          <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="flex size-9 items-center justify-center rounded-xl bg-orange-50 text-orange-600"><BarChart3 className="size-4" /></div><div><h2 className="text-sm font-bold text-slate-950">{t("finance.tabs.flow")}</h2><p className="mt-0.5 text-[10px] text-slate-400">{t("finance.chart.title")}</p></div></div><div className="hidden items-center gap-3 text-[9px] sm:flex"><Legend color="bg-orange-600" label="Receitas" /><Legend color="bg-yellow-400" label="Despesas" /></div></div>
...
function EntryDetails({ entry, onClose, onPay, onReverse }: { entry: FinancialEntry; onClose: () => void; onPay: () => void; onReverse: () => void }) {
  ...
  const details: Array<[string, string]> = [
    ...
    ["Contato", entry.contact || t("finance.notInformed")],
  ...
  if (entry.competenceDate) details.push([t("finance.form.fields.competence"), formatDate(entry.competenceDate)]);
  if (entry.recurrence !== "NONE") details.push([t("finance.form.fields.recurrence"), `${recurrenceLabels[entry.recurrence]}${entry.recurrenceDay ? ` (${t("finance.form.fields.dueDay")}: ${entry.recurrenceDay})` : ""}`]);
  const canReverse = !entry.reversed && entry.status !== "CANCELLED" && !entry.reversalOfId;
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-orange-600">{t("finance.details.title")}</p><h2 className="mt-1 text-lg font-black text-slate-950">{entry.description}</h2><p className="mt-1 text-xs text-slate-500">{entry.code}</p></div><button type="button" onClick={onClose} aria-label={t("finance.actions.closeDetails")} className="flex size-9 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100"><X className="size-4" /></button></div><dl className="mt-5 grid gap-3 sm:grid-cols-2">{details.map(([label, value]) => <div key={label} className="rounded-xl bg-slate-50 p-3"><dt className="text-[10px] text-slate-400">{label}</dt><dd className="mt-1 truncate text-xs font-bold text-slate-800">{value}</dd></div>)}</dl>{entry.payments.length > 0 && <div className="mt-4"><h3 className="text-xs font-black text-slate-800">{t("finance.details.history")}</h3><div className="mt-2 divide-y divide-slate-100 rounded-xl border border-slate-200">{entry.payments.map((payment) => <div key={payment.id} className="p-3"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold text-slate-800">{paymentMethodLabels[payment.paymentMethod]}</p><p className="mt-0.5 text-[10px] text-slate-400">{formatDate(payment.paidAt)} · {payment.receivedByName}</p></div><strong className="text-xs text-green-600">{formatCurrency(payment.amount)}</strong></div>{(payment.interest > 0 || payment.discount > 0 || payment.remainingAfter != null) && <p className="mt-1.5 text-[10px] text-slate-500">{payment.interest > 0 ? `Juros +${formatCurrency(payment.interest)} ` : ""}{payment.discount > 0 ? `Desconto -${formatCurrency(payment.discount)} ` : ""}{payment.remainingAfter != null ? `· Saldo restante ${formatCurrency(payment.remainingAfter)}` : ""}</p>}</div>)}</div></div>}{entry.reversed && entry.reversedAt && <p className="mt-3 rounded-xl bg-red-50 p-3 text-xs text-red-700"><strong>Revertido:</strong> este lançamento foi estornado em {formatDate(entry.reversedAt)}.</p>}{entry.outstandingAmount > 0 && entry.status !== "CANCELLED" && !entry.reversed && <button type="button" onClick={onPay} className="mt-4 h-10 w-full rounded-xl bg-green-600 text-xs font-bold text-white">{t("finance.actions.registerPayment")}</button>}{canReverse && <button type="button" onClick={onReverse} className="mt-2 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-red-200 text-xs font-bold text-red-600 hover:bg-red-50"><RotateCcw className="size-3.5" />{t("finance.actions.reverseShort")}</button>}{entry.cancelReason && <p className="mt-3 rounded-xl bg-red-50 p-3 text-xs text-red-700"><strong>Cancelamento:</strong> {entry.cancelReason}</p>}</div></div>;
}
...
function ConfirmPayment({ entry, loading, onCancel, onConfirm }: { entry: FinancialEntry; loading: boolean; onCancel: () => void; onConfirm: (input: PaymentInput) => void }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-sm overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex size-10 items-center justify-center rounded-xl bg-green-50 text-green-600"><CheckCircle2 className="size-4" /></div><h2 className="mt-4 text-base font-black text-slate-950">{t("finance.actions.registerPayment")}</h2><p className="mt-2 text-xs leading-5 text-slate-500">Saldo em aberto de <strong className="text-slate-700">{formatCurrency(entry.outstandingAmount)}</strong> em {entry.description}.</p><div className="mt-4 space-y-3"><label className="block text-xs font-bold text-slate-700">{entry.type === "INCOME" ? "Valor recebido" : "Valor pago"}<input type="number" min="0.01" max={entry.outstandingAmount} step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label><div className="grid grid-cols-2 gap-3"><label className="block text-xs font-bold text-slate-700">{t("finance.details.interest")}<input type="number" min="0" step="0.01" value={interest} onChange={(event) => setInterest(event.target.value)} placeholder="0,00" className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">{t("finance.details.discount")}<input type="number" min="0" step="0.01" value={discount} onChange={(event) => setDiscount(event.target.value)} placeholder="0,00" className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label></div><p className="rounded-xl bg-slate-50 p-3 text-xs font-semibold text-slate-700">Valor efetivo: <strong className="text-green-600">{formatCurrency(effective)}</strong></p><label className="block text-xs font-bold text-slate-700">Forma de pagamento<select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as PaymentMethod)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm">{(["PIX", "CREDIT_CARD", "DEBIT_CARD", "CASH", "BOLETO", "CHECK", "STORE_CREDIT"] as PaymentMethod[]).map((method) => <option key={method} value={method}>{paymentMethodLabels[method]}</option>)}</select></label><label className="block text-xs font-bold text-slate-700">Data<input type="date" value={paidAt} onChange={(event) => setPaidAt(event.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">{t("finance.details.note")}<input value={notes} maxLength={500} onChange={(event) => setNotes(event.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label></div><div className="mt-5 flex justify-end gap-2"><button type="button" disabled={loading} onClick={onCancel} className="h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50">Cancelar</button><button type="button" disabled={loading || !valid || !paidAt} onClick={() => onConfirm({ amount: numericAmount, interest: numericInterest || undefined, discount: numericDiscount || undefined, paymentMethod, paidAt, notes: notes.trim() || undefined })} className="inline-flex h-10 items-center gap-2 rounded-xl bg-green-600 px-4 text-xs font-bold text-white hover:bg-green-700 disabled:opacity-50">{loading && <LoaderCircle className="size-3.5 animate-spin" />}Registrar</button></div></div></div>;
}
...
function ReverseEntry({ entry, loading, onCancel, onConfirm }: { entry: FinancialEntry; loading: boolean; onCancel: () => void; onConfirm: (reason: string) => void }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex size-10 items-center justify-center rounded-xl bg-red-50 text-red-600"><RotateCcw className="size-4" /></div><h2 className="mt-4 text-base font-black text-slate-950">{t("finance.actions.reverse")}</h2><p className="mt-2 text-xs leading-5 text-slate-500">Será criada uma <strong className="text-slate-700">reversão</strong> (tipo oposto) de {formatCurrency(entry.amount, locale)} — {entry.description}. O lançamento original ficará marcado como revertido e nunca será alterado.</p><label className="mt-4 block text-xs font-bold text-slate-700">{t("finance.details.reverseReason")}<textarea value={reason} maxLength={500} onChange={(event) => setReason(event.target.value)} placeholder="Ex.: cobrança duplicada, valor incorreto..." className="mt-1.5 h-24 w-full resize-none rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm" /></label><div className="mt-5 flex justify-end gap-2"><button type="button" disabled={loading} onClick={onCancel} className="h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50">Cancelar</button><button type="button" disabled={loading || !valid} onClick={() => onConfirm(reason.trim())} className="inline-flex h-10 items-center gap-2 rounded-xl bg-red-600 px-4 text-xs font-bold text-white hover:bg-red-700 disabled:opacity-50">{loading && <LoaderCircle className="size-3.5 animate-spin" />}Estornar</button></div></div></div>;
}

function FinancialStructure({ onClose }: { onClose: () => void }) {
  const t = useT();
  const [tab, setTab] = useState<"cost" | "category">("cost");
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-orange-600">{t("finance.tabs.structure")}</p><h2 className="mt-1 text-lg font-black text-slate-950">Centros de custo e plano de contas</h2></div><button type="button" onClick={onClose} aria-label="Fechar" className="flex size-9 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100"><X className="size-4" /></button></div><div className="mt-4 flex gap-2"><TabButton active={tab === "cost"} onClick={() => setTab("cost")} icon={<Tags className="size-3.5" />} label="Centros de custo" /><TabButton active={tab === "category"} onClick={() => setTab("category")} icon={<Layers3 className="size-3.5" />} label="Plano de contas" /></div>{tab === "cost" ? <CostCentersPanel /> : <AccountCategoriesPanel />}</div></div>;
}
...
function CostCentersPanel() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : t("finance.errors.createCostCenter"));
    }
  }
...
function AccountCategoriesPanel() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : t("finance.errors.createAccounting"));
    }
  }
