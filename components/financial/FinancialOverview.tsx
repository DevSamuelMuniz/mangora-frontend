...
export default function FinancialOverview() {
  ...
  async function confirmPayment(input: { amount: number; paymentMethod: PaymentMethod; paidAt: string; notes?: string }) {
    if (!entryToPay) return;
    try {
      setPaying(true);
      setError("");
      await apiRequest(`/financial/${entryToPay.id}/pay`, { method: "PATCH", body: JSON.stringify(input) });
      setEntryToPay(null);
      await loadEntries();
    } catch (requestError) {
    ...
    <>
      <section>
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">Gestão financeira</p><h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Financeiro</h1><p className="mt-1 text-xs text-slate-500">Acompanhe receitas, despesas e compromissos.</p></div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <SummaryCard icon={WalletCards} label="Saldo realizado" value={currencyFormatter.format(summary.balance)} description="Receitas menos despesas pagas" iconClassName="bg-orange-50 text-orange-600" />
          <SummaryCard icon={ArrowUpRight} label="Receitas realizadas" value={currencyFormatter.format(summary.paidIncome)} description="Entradas confirmadas" iconClassName="bg-green-50 text-green-600" />
          <SummaryCard icon={CircleDollarSign} label="Contas a receber" value={currencyFormatter.format(summary.receivable)} description="Receitas pendentes" iconClassName="bg-yellow-50 text-yellow-600" />
        ...
        <div className="mt-4 grid items-stretch gap-4 xl:grid-cols-[1.3fr_0.7fr]">
          <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            ...
            <div className="mt-6 flex h-48 items-end gap-3 sm:gap-5">
              {cashFlow.map((item) => <div key={item.key} className="flex h-full flex-1 flex-col items-center justify-end gap-2"><div className="flex h-full w-full items-end justify-center gap-1"><div title={currencyFormatter.format(item.income)} className="w-full max-w-5 rounded-t-md bg-orange-600" style={{ height: `${Math.max(item.income ? 3 : 0, item.income / chartMaximum * 100)}%` }} /><div title={currencyFormatter.format(item.expense)} className="w-full max-w-5 rounded-t-md bg-yellow-400" style={{ height: `${Math.max(item.expense ? 3 : 0, item.expense / chartMaximum * 100)}%` }} /></div><span className="text-[9px] font-semibold capitalize text-slate-400">{item.month}</span></div>)}
            </div>
          </article>
          ...
          <article className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {pendingEntries.length ? pendingEntries.map((entry) => <div key={entry.id} className="flex items-center gap-3 px-4 py-3 sm:px-5"><div className={`flex size-8 shrink-0 items-center justify-center rounded-xl ${entry.displayStatus === "OVERDUE" ? "bg-red-50 text-red-600" : entry.type === "INCOME" ? "bg-green-50 text-green-600" : "bg-amber-50 text-amber-600"}`}>{entry.type === "INCOME" ? <ArrowUpRight className="size-3.5" /> : <ArrowDownRight className="size-3.5" />}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-slate-800">{entry.description}</p><p className="mt-1 text-[9px] text-slate-400">Vence em {formatDate(entry.dueDate)}</p></div><div className="shrink-0 text-right"><p className={`text-xs font-black ${entry.type === "INCOME" ? "text-green-600" : "text-slate-800"}`}>{currencyFormatter.format(entry.amount)}</p><p className={`mt-1 text-[9px] font-bold ${entry.displayStatus === "OVERDUE" ? "text-red-600" : "text-amber-600"}`}>{financialStatusLabels[entry.displayStatus]}</p></div></div>) : <p className="px-5 py-8 text-center text-xs text-slate-400">Nenhum compromisso pendente.</p>}
            </div>
          </article>
        </div>
      ...
      {selectedEntry && <EntryDetails entry={selectedEntry} onClose={() => setSelectedEntry(null)} onPay={() => { setEntryToPay(selectedEntry); setSelectedEntry(null); }} />}
      {entryToPay && <ConfirmPayment entry={entryToPay} loading={paying} onCancel={() => setEntryToPay(null)} onConfirm={(input) => void confirmPayment(input)} />}
    </>
  );
}
...
function ConfirmPayment({ entry, loading, onCancel, onConfirm }: { entry: FinancialEntry; loading: boolean; onCancel: () => void; onConfirm: (input: { amount: number; paymentMethod: PaymentMethod; paidAt: string; notes?: string }) => void }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex size-10 items-center justify-center rounded-xl bg-green-50 text-green-600"><CheckCircle2 className="size-4" /></div><h2 className="mt-4 text-base font-black text-slate-950">Registrar pagamento</h2><p className="mt-2 text-xs leading-5 text-slate-500">Saldo em aberto de <strong className="text-slate-700">{currencyFormatter.format(entry.outstandingAmount)}</strong> em {entry.description}.</p><div className="mt-4 space-y-3"><label className="block text-xs font-bold text-slate-700">{entry.type === "INCOME" ? "Valor recebido" : "Valor pago"}<input type="number" min="0.01" max={entry.outstandingAmount} step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Forma de pagamento<select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as PaymentMethod)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm">{(["PIX", "CREDIT_CARD", "DEBIT_CARD", "CASH", "BOLETO", "CHECK", "STORE_CREDIT"] as PaymentMethod[]).map((method) => <option key={method} value={method}>{paymentMethodLabels[method]}</option>)}</select></label><label className="block text-xs font-bold text-slate-700">Data<input type="date" value={paidAt} onChange={(event) => setPaidAt(event.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Observação (opcional)<input value={notes} maxLength={500} onChange={(event) => setNotes(event.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label></div><div className="mt-5 flex justify-end gap-2"><button type="button" disabled={loading} onClick={onCancel} className="h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50">Cancelar</button><button type="button" disabled={loading || !valid || !paidAt} onClick={() => onConfirm({ amount: numericAmount, paymentMethod, paidAt, notes: notes.trim() || undefined })} className="inline-flex h-10 items-center gap-2 rounded-xl bg-green-600 px-4 text-xs font-bold text-white hover:bg-green-700 disabled:opacity-50">{loading && <LoaderCircle className="size-3.5 animate-spin" />}Registrar</button></div></div></div>;
}
