    ...
    <>
      <section>
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          ...
          <Link href="/financeiro/novo" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-sm font-bold text-white shadow-lg shadow-orange-200 transition hover:-translate-y-0.5 hover:shadow-xl"><Plus className="size-4" />Novo lançamento</Link>
        </div>
...
function EntryDetails({ entry, onClose }: { entry: FinancialEntry; onClose: () => void }) { const details = [["Tipo", financialTypeLabels[entry.type]], ["Categoria", entry.category], ["Contato", entry.contact || "Não informado"], ["Vencimento", formatDate(entry.dueDate)], ["Conta", entry.account], ["Valor", currencyFormatter.format(entry.amount)]]; return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-orange-600">Detalhes do lançamento</p><h2 className="mt-1 text-lg font-black text-slate-950">{entry.description}</h2><p className="mt-1 text-xs text-slate-500">{entry.code}</p></div><button type="button" onClick={onClose} aria-label="Fechar detalhes" className="flex size-9 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100"><X className="size-4" /></button></div><dl className="mt-5 grid gap-3 sm:grid-cols-2">{details.map(([label, value]) => <div key={label} className="rounded-xl bg-slate-50 p-3"><dt className="text-[10px] text-slate-400">{label}</dt><dd className="mt-1 truncate text-xs font-bold text-slate-800">{value}</dd></div>)}</dl>{entry.cancelReason && <p className="mt-3 rounded-xl bg-red-50 p-3 text-xs text-red-700"><strong>Cancelamento:</strong> {entry.cancelReason}</p>}</div></div>; }
function ConfirmPayment({ entry, loading, onCancel, onConfirm }: { entry: FinancialEntry; loading: boolean; onCancel: () => void; onConfirm: () => void }) { return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="alertdialog" aria-modal="true" className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex size-10 items-center justify-center rounded-xl bg-green-50 text-green-600"><CheckCircle2 className="size-4" /></div><h2 className="mt-4 text-base font-black text-slate-950">Marcar como pago?</h2><p className="mt-2 text-xs leading-5 text-slate-500">O lançamento <strong className="text-slate-700">{entry.description}</strong> entrará no saldo realizado.</p><div className="mt-5 flex justify-end gap-2"><button type="button" disabled={loading} onClick={onCancel} className="h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50">Cancelar</button><button type="button" disabled={loading} onClick={onConfirm} className="inline-flex h-10 items-center gap-2 rounded-xl bg-green-600 px-4 text-xs font-bold text-white hover:bg-green-700 disabled:opacity-50">{loading && <LoaderCircle className="size-3.5 animate-spin" />}Confirmar pagamento</button></div></div></div>; }
