...
export default function StockOverview() {
  ...
  return (
    <section>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">Operação</p><h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Estoque</h1><p className="mt-1 text-xs text-slate-500">Controle saldos, níveis mínimos e movimentações.</p></div><Link href="/estoque?acao=movimentar" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-sm font-bold text-white shadow-lg shadow-orange-200 transition hover:-translate-y-0.5 hover:shadow-xl"><ArrowRightLeft className="size-4" />Nova movimentação</Link></div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryCard icon={CircleDollarSign} label="Valor estimado" value={currencyFormatter.format(overview.summary.inventoryValue)} iconClassName="bg-yellow-50 text-yellow-600" />
...
function getSituationStyle(situation: Exclude<StockSituation, "all">) { if (situation === "out") return { label: "Sem estoque", textClassName: "text-red-600", barClassName: "bg-red-500" }; if (situation === "low") return { label: "Estoque baixo", textClassName: "text-amber-600", barClassName: "bg-amber-500" }; return { label: "Estoque normal", textClassName: "text-green-600", barClassName: "bg-green-500" }; }
