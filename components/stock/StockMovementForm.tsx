...
export default function StockMovementForm({ initialProductId = "" }: { initialProductId?: string }) {
  ...
  return (
    <section className="mx-auto max-w-5xl">
      <div className="flex items-start gap-3"><Link href="/estoque" aria-label="Voltar para estoque" className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:bg-slate-50 hover:text-orange-600"><ArrowLeft className="size-4" /></Link><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">Operação</p><h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Nova movimentação</h1><p className="mt-1 text-xs text-slate-500">Registre uma entrada, saída ou ajuste de saldo.</p></div></div>
        ...
        <div className="grid items-start gap-4 lg:grid-cols-[1fr_0.7fr]">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Observações (opcional)" id="notes" className="sm:col-span-2"><textarea id="notes" name="notes" rows={3} maxLength={1000} placeholder="Detalhes adicionais..." className="w-full resize-y rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-orange-400 focus:ring-4 focus:ring-orange-100" /></Field>
            </div>
          </div>
...
function Balance({ label, value, highlight = false }: { label: string; value: number; highlight?: boolean }) { return <div className={`rounded-xl p-3 ${highlight ? "bg-orange-50" : "bg-slate-50"}`}><p className="text-[9px] text-slate-400">{label}</p><p className={`mt-1 text-xl font-black ${highlight ? "text-orange-700" : "text-slate-800"}`}>{value}</p><p className="text-[9px] text-slate-400">unidades</p></div>; }
