...
export default function StockTransferForm({ initialProductId }: { initialProductId?: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/estoque?toast=${encodeURIComponent(t("workspace.transfer.done"))}`);
      router.refresh();
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : t("workspace.transfer.failed")); }
  }

  if (loading) return <div className="flex min-h-48 items-center justify-center"><LoaderCircle className="size-5 animate-spin text-orange-600" /><span className="ml-2 text-xs font-bold text-slate-600">{t("workspace.transfer.saving")}</span></div>;
  if (!group?.group || destinations.length === 0) return <div className="rounded-2xl border border-amber-300 bg-amber-50 p-5 text-center"><Store className="mx-auto size-6 text-amber-700" /><h2 className="mt-3 text-sm font-black text-amber-950">{t("workspace.transfer.addStoreFirst")}</h2><p className="mt-1 text-xs text-amber-800">{t("workspace.transfer.needTwoStores")}</p><button type="button" onClick={() => router.push("/unidades")} className="mt-4 h-10 rounded-xl bg-amber-700 px-4 text-xs font-black text-white">{t("workspace.transfer.manageStores")}</button></div>;

  return <form onSubmit={(event) => void submit(event)}>
    <div className="flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-2xl bg-orange-100 text-orange-700"><ArrowRightLeft className="size-5" /></div><div><p className="text-[10px] font-black uppercase tracking-wider text-orange-600">{t("workspace.transfer.eyebrow")}</p><h2 className="text-xl font-black text-slate-950">{t("workspace.transfer.title")}</h2></div></div>
    ...
    <div className="mt-5 grid gap-4 sm:grid-cols-2">
      <label className="text-xs font-bold text-slate-700">{t("workspace.transfer.source")}<select required value={productId} onChange={(event) => setProductId(event.target.value)} className={inputClass}><option value="">{t("workspace.transfer.selectProduct")}</option>{stock?.products.filter((product) => product.active && product.stock - product.reservedStock > 0).map((product) => <option key={product.id} value={product.id}>{product.name} · {product.stock - product.reservedStock} disp.</option>)}</select></label>
      <label className="text-xs font-bold text-slate-700">{t("workspace.transfer.destination")}<select required value={destinationCompanyId} onChange={(event) => setDestinationCompanyId(event.target.value)} className={inputClass}>{destinations.map((unit) => <option key={unit.company.id} value={unit.company.id}>{unit.company.unitCode} · {unit.company.tradeName}</option>)}</select></label>
      <label className="text-xs font-bold text-slate-700">{t("workspace.transfer.quantity")}<input required min={1} max={availableStock || 1} type="number" value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} className={inputClass} /><span className="mt-1 block text-[9px] font-medium text-slate-500">Disponível: {availableStock} unidade(s)</span></label>
