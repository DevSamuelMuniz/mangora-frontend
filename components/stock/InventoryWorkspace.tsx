  ...
  return (
    <section className="mx-auto max-w-5xl">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">{t("stock.inventory.eyebrow")}</p>
          <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">{t("stock.inventory.title")}</h1>
          <p className="mt-1 text-xs text-slate-500">{t("stock.inventory.subtitle")}</p>
        </div>
        <button type="button" onClick={() => setShowNew(true)} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-sm font-bold text-white shadow-lg shadow-orange-200 transition hover:-translate-y-0.5 hover:shadow-xl"><Plus className="size-4" />{t("stock.inventory.new")}</button>
      </div>
            ...
            <article key={count.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <button type="button" onClick={() => count.status === "DRAFT" ? setEditing(count) : setEditing(editing?.id === count.id ? null : count)} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left hover:bg-slate-50 sm:px-5">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-xs font-black text-slate-800">{count.name ?? t("stock.inventory.countFallback", { code: count.id.slice(0, 6).toUpperCase() })}</p><p className="mt-0.5 text-[10px] text-slate-400">{formatDate(count.createdAt)} · {count.createdByName} · {count.items.length} item(ns)</p></div>
                </div>
...
function CountEditor({ count, onDone, onError }: { count: StockCount; onDone: (message: string) => void; onError: (message: string) => void }) {
  ...
  async function saveAndComplete() {
    ...
    if (!items.length) { onError(t("stock.inventory.errors.empty")); return; }
    try {
      await addItems.mutateAsync({ id: count.id, items });
      await complete.mutateAsync(count.id);
      onDone(t("stock.inventory.completed"));
    } catch (requestError) {
      onError(requestError instanceof Error ? requestError.message : t("stock.inventory.errors.complete"));
    }
  }
...
function NewCountDialog({ onClose, onDone, onError }: { onClose: () => void; onDone: (message: string) => void; onError: (message: string) => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      onClose();
      onDone(t("stock.inventory.started", { code: count.id.slice(0, 6).toUpperCase() }));
    } catch (requestError) {
      onError(requestError instanceof Error ? requestError.message : t("stock.inventory.errors.create"));
    }
  }
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-orange-600">{t("stock.inventory.title")}</p><h2 className="mt-1 text-lg font-black text-slate-950">{t("stock.inventory.new")}</h2></div><button type="button" onClick={onClose} aria-label="Fechar" className="flex size-9 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100"><X className="size-4" /></button></div><form onSubmit={(event) => void submit(event)} className="mt-4 space-y-3"><label className="block text-xs font-bold text-slate-700">{t("stock.inventory.newDialog.name")}<input value={name} onChange={(event) => setName(event.target.value)} placeholder={t("stock.inventory.newDialog.namePlaceholder")} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">{t("stock.inventory.newDialog.notes")}<textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={2} placeholder={t("stock.inventory.newDialog.notesPlaceholder")} className="mt-1.5 w-full resize-none rounded-xl border border-slate-200 p-3 text-xs" /></label><div className="flex justify-end gap-2 pt-2"><button type="button" onClick={onClose} className="h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-slate-600 hover:bg-slate-50">{t("stock.inventory.newDialog.cancel")}</button><button type="submit" disabled={create.isPending} className="inline-flex h-10 items-center gap-2 rounded-xl bg-orange-600 px-4 text-xs font-bold text-white disabled:opacity-50">{create.isPending && <LoaderCircle className="size-3.5 animate-spin" />}Criar contagem</button></div></form></div></div>;
}

function Empty() {
  const t = useT();
  return <div className="mt-5 flex min-h-64 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-white p-8 text-center"><PackageCheck className="size-8 text-slate-300" /><h2 className="mt-3 text-sm font-black text-slate-900">{t("stock.inventory.emptyState.title")}</h2><p className="mt-1 max-w-sm text-xs text-slate-500">{t("stock.inventory.emptyState.hint")}</p></div>;
}
