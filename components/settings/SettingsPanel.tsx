...
function OnlineStoreForm({ company }: { company: CompanySettings }) { return <div className="space-y-5"><div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-orange-100 bg-orange-50 p-4"><div><p className="text-xs font-black text-orange-950">mangora.com.br/loja/{company.slug}</p><p className="mt-1 text-[10px] text-orange-700">Endereço público baseado no identificador da empresa.</p></div><Link href={`/loja/${company.slug}`} target="_blank" className="flex h-9 items-center gap-2 rounded-xl bg-white px-3 text-[10px] font-bold text-orange-700 shadow-sm"><ExternalLink className="size-3.5" />Visualizar página</Link></div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-3 py-1 text-[10px] font-black ${company.publicPageEnabled ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>{company.publicPageEnabled ? "● Página publicada" : "○ Página desativada"}</span>{company.publicPageEnabled && !company.publicPickupEnabled && !company.publicDeliveryEnabled && <span className="rounded-full bg-amber-100 px-3 py-1 text-[10px] font-bold text-amber-700">Sem retirada nem entrega — a página fica indisponível para pedidos</span>}{!company.publicPageEnabled && <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] text-slate-500">Ative &ldquo;Ativar página online&rdquo; para publicar o catálogo.</span>}</div><SettingsGroup title="Disponibilidade"><Toggle name="publicPageEnabled" title="Ativar página online" description="Publica o catálogo e permite receber pedidos." defaultChecked={company.publicPageEnabled} /><Toggle name="publicPickupEnabled" title="Permitir retirada" description="Cliente retira o pedido no estabelecimento." defaultChecked={company.publicPickupEnabled} /><Toggle name="publicDeliveryEnabled" title="Permitir entrega" description="A empresa combinará a entrega com o cliente." defaultChecked={company.publicDeliveryEnabled} /></SettingsGroup><div className="grid gap-4 sm:grid-cols-2"><Field label="WhatsApp público" id="publicWhatsapp"><input id="publicWhatsapp" name="publicWhatsapp" inputMode="tel" defaultValue={company.publicWhatsapp ?? ""} placeholder="(81) 99999-9999" className={inputClass} /></Field><Field label="Descrição pública" id="publicDescription"><textarea id="publicDescription" name="publicDescription" rows={3} maxLength={500} defaultValue={company.publicDescription ?? ""} placeholder="Conte aos clientes o que sua empresa oferece." className="w-full resize-none rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-100" /></Field></div><p className="text-[10px] leading-4 text-slate-400">A visibilidade de cada item é controlada no cadastro do produto. Produtos sem estoque disponível não aparecem ao público.</p></div>; }
