...
function SalesForm({ company }: { company: CompanySettings }) { const t = useT(); return <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="Pagamento padrão" id="defaultPayment"><select id="defaultPayment" name="defaultPayment" defaultValue={company.defaultPayment} className={inputClass}><option value="PIX">PIX</option><option value="CASH">Dinheiro</option><option value="DEBIT_CARD">Cartão de débito</option><option value="CREDIT_CARD">Cartão de crédito</option><option value="BOLETO">Boleto</option></select></Field><Field label="Desconto máximo (%)" id="maximumDiscount"><input id="maximumDiscount" name="maximumDiscount" type="number" min={0} max={100} step="0.5" defaultValue={company.maximumDiscount} className={inputClass} /></Field></div><SettingsGroup title="Regras aplicadas"><Toggle name="requireCustomer" title="Exigir cliente identificado" description={t("settings.panel.sales.requireCustomerHint")} defaultChecked={company.requireCustomer} /><Toggle name="allowPendingSales" title="Permitir vendas pendentes" description={t("settings.panel.notifications.receivablesNote")} defaultChecked={company.allowPendingSales} /><Toggle name="allowNegativeStock" title="Permitir estoque negativo" description="Permite vender/pedir mais do que o disponível (desligado: bloqueia a operação)." defaultChecked={company.allowNegativeStock} /></SettingsGroup></div>; }
function NotificationsForm({ company }: { company: CompanySettings }) { const t = useT(); return <div className="space-y-5"><SettingsGroup title="Alertas no sistema"><Toggle name="lowStockNotification" title="Estoque baixo" description={t("settings.panel.notifications.lowStockHint")} defaultChecked={company.lowStockNotification} /><Toggle name="overdueAccountNotification" title="Contas vencidas" description={t("settings.panel.notifications.overdueHint")} defaultChecked={company.overdueAccountNotification} /><Toggle name="saleNotification" title="Novas vendas" description={t("settings.panel.notifications.digestHint")} defaultChecked={company.saleNotification} /></SettingsGroup><div className="grid gap-4 sm:grid-cols-2"><Field label="E-mail para resumos" id="summaryEmail"><input id="summaryEmail" name="summaryEmail" type="email" defaultValue={company.summaryEmail ?? ""} className={inputClass} /></Field><Field label="Frequência" id="summaryFrequency"><select id="summaryFrequency" name="summaryFrequency" defaultValue={company.summaryFrequency} className={inputClass}><option value="daily">{t("settings.panel.notifications.daily")}</option><option value="weekly">{t("settings.panel.notifications.weekly")}</option><option value="disabled">{t("settings.panel.notifications.disabled")}</option></select></Field></div><p className="text-[10px] text-slate-400">Os resumos são processados automaticamente, sem duplicação, e os e-mails com falha entram em reenvio gradual.</p></div>; }
...
function SecurityForm({ company, saving }: { company: CompanySettings; saving: boolean }) {
  ...
  return <div className="space-y-5">
    <div className="grid gap-4 sm:grid-cols-2"><Field label={t("settings.panel.security.sessionTimeout")} id="sessionTimeout"><input id="sessionTimeout" name="sessionTimeout" type="number" min={5} max={480} step={5} defaultValue={company.sessionTimeout} className={inputClass} /></Field><Field label="Tentativas antes do bloqueio" id="loginAttempts"><input id="loginAttempts" name="loginAttempts" type="number" min={3} max={10} defaultValue={company.loginAttempts} className={inputClass} /></Field></div>
    ...
    <SettingsGroup title={t("settings.panel.security.confirmations")}>
      <Toggle name="requirePasswordForRecords" title="Cadastros e edições" description={t("settings.panel.security.recordsHint")} defaultChecked={company.requirePasswordForRecords} />
      <Toggle name="requirePasswordForStock" title="Alterações no estoque" description={t("settings.panel.security.stockHint")} defaultChecked={company.requirePasswordForStock} />
      <Toggle name="requirePasswordForSaleReturns" title="Cancelamento de itens vendidos" description={t("settings.panel.security.returnsHint")} defaultChecked={company.requirePasswordForSaleReturns} />
    </SettingsGroup>
    ...
    <section className="rounded-xl border border-slate-200 p-4"><div className="flex items-center gap-2"><KeyRound className="size-4 text-orange-600" /><h3 className="text-xs font-bold text-slate-800">{t("settings.panel.security.passwordTitle")}</h3></div><div className="mt-4 grid gap-4 sm:grid-cols-3"><Field label="Senha atual" id="currentPassword"><input id="currentPassword" type="password" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} className={inputClass} /></Field><Field label="Nova senha" id="newPassword"><input id="newPassword" type="password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} className={inputClass} /></Field><Field label="Confirmar nova senha" id="passwordConfirmation"><input id="passwordConfirmation" type="password" autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} className={inputClass} /></Field></div><button type="button" disabled={busy || !currentPassword || !newPassword} onClick={() => void changePassword()} className="mt-3 h-10 rounded-xl bg-orange-600 px-4 text-xs font-bold text-white disabled:opacity-50">{t("settings.panel.security.passwordAction")}</button></section>
