...
export default function SettingsPanel({ initialTab = "company" }: { initialTab?: SettingsTab }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (cause) { setActionError(cause instanceof Error ? cause.message : "Não foi possível salvar as configurações."); }
  }
...
function OnlineStoreForm({ company }: { company: CompanySettings }) {
  const noFulfillment = company.publicPageEnabled && !company.publicPickupEnabled && !company.publicDeliveryEnabled;
  return <div className="space-y-5">
    ...
    <SettingsGroup title="Identidade da página"><div className="grid gap-4 px-4 py-3 sm:grid-cols-2">
      <Field label="Logo da página (link)" id="publicLogoUrl"><input id="publicLogoUrl" name="publicLogoUrl" defaultValue={company.publicLogoUrl ?? ""} placeholder="https://…/logo.png" className={inputClass} /></Field>
      <Field label="Imagem de capa (link)" id="publicCoverUrl"><input id="publicCoverUrl" name="publicCoverUrl" defaultValue={company.publicCoverUrl ?? ""} placeholder="https://…/capa.png" className={inputClass} /></Field>
      <div>{company.publicCoverUrl && <><p className="mb-1.5 text-xs font-bold text-slate-700">Prévia da capa</p><div className="h-24 w-full rounded-xl border border-slate-200 bg-slate-100" style={{ backgroundImage: `url(${company.publicCoverUrl})`, backgroundSize: "cover", backgroundPosition: "center" }} /></>}</div>
    </div></SettingsGroup>

    <SettingsGroup title="Cores da página"><ColorControl name="publicBrandColor" label="Cor principal" description="Botões, preços e destaques." defaultValue={company.publicBrandColor} /><ColorControl name="publicTitleColor" label="Cor do título" description="Nome da loja e títulos da página." defaultValue={company.publicTitleColor} /><ColorControl name="publicTextColor" label="Cor do texto" description="Textos da página." defaultValue={company.publicTextColor} /><ColorControl name="publicBackgroundColor" label="Cor de fundo" description="Fundo da página." defaultValue={company.publicBackgroundColor} /><ColorControl name="publicPanelColor" label="Cor das superfícies" description="Cards e painel do pedido." defaultValue={company.publicPanelColor} /></SettingsGroup>
    ...
    <p className="text-[10px] leading-4 text-slate-400">A visibilidade de cada item é controlada no cadastro do produto. Produtos sem estoque disponível não aparecem ao público. Use links públicos de imagem (logo/capa) — PNG/JPG funcionam melhor.</p>
  </div>;
}
...
function SecurityForm({ company }: { company: CompanySettings }) {
  ...
  async function changePassword() {
    ...
    try { setFeedback(""); await changePasswordMutation.mutateAsync({ currentPassword, newPassword }); setCurrentPassword(""); setNewPassword(""); setConfirmation(""); setFeedback("Senha alterada. As demais sessões foram encerradas."); }
    catch (cause) { setFeedback(cause instanceof Error ? cause.message : "Não foi possível alterar a senha."); }
  }
  async function revokeSession(id: string) {
    try { await deleteSessionMutation.mutateAsync({ id }); setFeedback("Sessão encerrada."); }
    catch (cause) { setFeedback(cause instanceof Error ? cause.message : "Não foi possível encerrar a sessão."); }
  }
  async function revokeOthers() {
    try { const result = await revokeOthersMutation.mutateAsync(); setFeedback(`${result.revoked} outra(s) sessão(ões) encerrada(s).`); }
  ...
  async function runJob(path: "email" | "summaries") {
    try { setFeedback(""); await runJobMutation.mutateAsync({ path }); setFeedback(path === "email" ? "Fila de e-mails processada." : "Resumo e alertas processados."); }
...
function ColorControl({ name, label, description, defaultValue }: { name: string; label: string; description: string; defaultValue: string | null }) {
  const [value, setValue] = useState(defaultValue ?? "");
  return <div className="flex items-center justify-between gap-4 px-4 py-3">
    <div><p className="text-xs font-bold text-slate-700">{label}</p><p className="text-[10px] text-slate-400">{description}</p></div>
    <div className="flex items-center gap-2">
      <input type="color" value={/^#[0-9a-f]{6}$/i.test(value) ? value : "#ff6b1a"} onChange={(event) => setValue(event.target.value)} aria-label={`Seletor de cor de ${label}`} className="h-9 w-12 cursor-pointer rounded-lg border border-slate-200 bg-white p-0.5" />
      <input name={name} value={value} onChange={(event) => setValue(event.target.value)} placeholder="#RRGGBB" className="h-9 w-28 rounded-lg border border-slate-200 bg-white px-2 font-mono text-xs outline-none focus:border-orange-400" />
      {value ? <button type="button" onClick={() => setValue("")} className="text-[10px] font-bold text-slate-400 transition hover:text-red-500">Limpar</button> : <span className="text-[9px] text-slate-300">tema</span>}
    </div>
  </div>;
}
