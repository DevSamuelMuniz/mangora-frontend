...
function OnlineStoreForm({ company }: { company: CompanySettings }) {
  const [brandColor, setBrandColor] = useState(company.publicBrandColor || "#ff6b1a");
  const noFulfillment = company.publicPageEnabled && !company.publicPickupEnabled && !company.publicDeliveryEnabled;
  const presets = ["#ff6b1a", "#e8550d", "#147a45", "#123d2b", "#7c3aed", "#2563eb", "#0ea5e9", "#db2777", "#b91c1c", "#ca8a04"];
  return <div className="space-y-5">
    ...
    <SettingsGroup title="Identidade da página"><div className="grid gap-4 px-4 py-3 sm:grid-cols-2">
      <div><label htmlFor="publicBrandColor" className="mb-1.5 block text-xs font-bold text-slate-700">Cor da marca</label><div className="flex items-center gap-2"><input id="publicBrandColor" name="publicBrandColor" type="color" value={brandColor} onChange={(event) => setBrandColor(event.target.value)} className="h-11 w-14 cursor-pointer rounded-lg border border-slate-200 bg-white p-1" /><div className="flex flex-wrap gap-1.5">{presets.map((color) => <button key={color} type="button" aria-label={`Cor ${color}`} onClick={() => setBrandColor(color)} className="size-7 rounded-lg border border-slate-200 transition hover:scale-110" style={{ backgroundColor: color }} />)}</div></div><p className="mt-1.5 text-[10px] text-slate-400">Cor que destaca botões e preços na sua página.</p></div>
