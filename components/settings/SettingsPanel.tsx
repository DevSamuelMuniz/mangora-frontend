...
function SecurityForm({ company, saving }: { company: CompanySettings; saving: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <SettingsGroup title="Confirmação de senha nas operações">
      <Toggle name="requirePasswordForRecords" title="Cadastros e edições" description="Exige a senha do usuário ao criar, editar ou excluir cadastros principais." defaultChecked={company.requirePasswordForRecords} />
      <Toggle name="requirePasswordForStock" title="Alterações no estoque" description="Exige senha em movimentações, transferências, inventários e lotes." defaultChecked={company.requirePasswordForStock} />
      <Toggle name="requirePasswordForSaleReturns" title="Cancelamento de itens vendidos" description="Exige senha antes de devolver ou retirar itens de uma venda." defaultChecked={company.requirePasswordForSaleReturns} />
    </SettingsGroup>
    <div className="flex justify-end">
      <button type="submit" disabled={saving} className="flex h-10 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-xs font-bold text-white shadow-sm shadow-orange-200 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0">
        {saving ? <><LoaderCircle className="size-4 animate-spin" />Salvando...</> : <><Save className="size-4" />Salvar confirmações</>}
      </button>
    </div>
    ...
    <section className="rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><Monitor className="size-4 text-orange-600" /><div><h3 className="text-xs font-bold text-slate-800">Sessões e dispositivos</h3><p className="text-[10px] text-slate-400">Acessos da sua conta em todas as empresas.</p></div></div><button type="button" disabled={busy || sessions.length < 2} onClick={() => void revokeOthers()} className="h-9 rounded-xl border border-slate-200 px-3 text-[10px] font-bold text-slate-600 disabled:opacity-40">Encerrar outras sessões</button></div><div className="mt-3 divide-y divide-slate-100">{sessions.map((session) => <div key={session.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="truncate text-xs font-bold text-slate-700">{session.deviceName || session.userAgent || "Navegador não identificado"}</p><p className="mt-1 text-[10px] text-slate-400">{session.ipAddress || "IP não informado"} · início {formatDateTime(session.createdAt)} · expira {formatDateTime(session.expiresAt)}</p></div>{session.current ? <span className="rounded-full bg-green-50 px-2 py-1 text-[9px] font-bold text-green-700">Sessão atual</span> : <button type="button" disabled={busy} onClick={() => void revokeSession(session.id)} className="text-[10px] font-bold text-red-600">Encerrar</button>}</div>)}</div></section>
