...
export default function BankReconciliation() {
  ...
  async function runReconcile() {
    ...
    try {
      ...
      setNotice(t("bank.match.autoDone", { matched: result.matched, pending: result.unmatched }));
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : t("bank.match.autoFailed"));
    }
  }
  ...
  return (
    <section className="mx-auto max-w-6xl">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">{t("bank.eyebrow")}</p>
          <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">{t("bank.title")}</h1>
          <p className="mt-1 text-xs text-slate-500">{t("bank.accounts.subtitle")}</p>
        </div>
        <button type="button" onClick={() => setShowNew(true)} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-sm font-bold text-white shadow-lg shadow-orange-200 transition hover:-translate-y-0.5 hover:shadow-xl"><Plus className="size-4" />{t("bank.actions.create")}</button>
      </div>

      {notice && <div role="status" className="mt-4 flex items-center justify-between rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-xs font-semibold text-green-700"><span>{notice}</span><button type="button" onClick={() => setNotice("")} className="font-bold underline">{t("bank.actions.close")}</button></div>}
      ...
      {loadingAccounts ? <div className="mt-5 flex min-h-40 items-center justify-center text-xs font-semibold text-slate-500"><LoaderCircle className="mr-2 size-5 animate-spin text-orange-600" />{t("bank.accounts.loading")}</div> : !(accounts ?? []).length ? <EmptyAccounts onCreate={() => setShowNew(true)} /> : (
        <>
          ...
          {account && <AccountWorkspace
            ...
            onImport={(format, content) => {
              ...
              importStatement.mutateAsync({ accountId: account.id, format, content })
                ...
                .catch((requestError) => setError(requestError instanceof Error ? requestError.message : t("bank.statement.importFailed")));
            }}
            ...
            onMatch={(transactionId, financialEntryId) => {
              ...
              matchTransaction.mutateAsync({ accountId: account.id, transactionId, financialEntryId })
                .then(() => setNotice(t("bank.match.matched")))
                .catch((requestError) => setError(requestError instanceof Error ? requestError.message : t("bank.match.failed")));
            }}
            onIgnore={(transactionId) => {
              setError("");
              ignoreTransaction.mutateAsync({ accountId: account.id, transactionId })
                .then(() => setNotice(t("bank.ignore.done")))
                .catch((requestError) => setError(requestError instanceof Error ? requestError.message : t("bank.ignore.failed")));
            }}
          />}
        </>
      )}
...
function AccountCard({ account, selected, onClick }: { account: BankAccount; selected: boolean; onClick: () => void }) {
  ...
  return <button type="button" onClick={onClick} className={`rounded-2xl border bg-white p-4 text-left shadow-sm transition ${selected ? `border-orange-300 ring-2 ${config.ring}` : "border-slate-200 hover:border-slate-300"}`}>
    ...
    <p className="text-sm font-black tabular-nums text-slate-900">{formatCurrency(account.openingBalance, locale)}</p>
  </button>;
}
  ...
  return (
    <div className="mt-5 space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="rounded-xl bg-slate-50 px-3 py-2 text-right"><p className="text-[10px] font-semibold text-slate-400">Saldo conforme extrato</p><p className="text-sm font-black tabular-nums text-slate-900">{formatCurrency(balance, locale)}</p></div>
            <button type="button" onClick={onToggleImport} className={`inline-flex h-10 items-center gap-2 rounded-xl border px-3.5 text-xs font-bold transition ${showImport ? "border-orange-300 bg-orange-50 text-orange-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}><Upload className="size-3.5" />{t("bank.actions.import")}</button>
      ...
      <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-4 py-4 sm:px-5"><h2 className="text-sm font-bold text-slate-950">{t("bank.statement.imported")}</h2><p className="mt-0.5 text-[10px] text-slate-400">{t("bank.subtitle")}</p></div>
        {transactions.length ? <div className="divide-y divide-slate-100">{transactions.map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} entries={openEntries} onMatch={(entryId) => onMatch(transaction.id, entryId)} onIgnore={() => onIgnore(transaction.id)} />)}</div> : <div className="flex min-h-44 flex-col items-center justify-center p-8 text-center"><FileText className="size-6 text-slate-300" /><p className="mt-3 text-xs text-slate-400">{t("bank.statement.empty")}</p><p className="mt-1 text-[11px] text-slate-400">{t("bank.statement.hint")}</p></div>}
      </article>
    </div>
  );
}
...
function ImportPanel({ onImport, importing }: { onImport: (format: "ofx" | "csv", content: string) => void; importing: boolean }) {
  ...
  return <div className="mt-5 rounded-2xl border-2 border-dashed border-amber-200 bg-amber-50/50 p-4 sm:p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs font-black text-slate-800">{t("bank.statement.title")}</p><div className="flex gap-2">{(["ofx", "csv"] as const).map((value) => <button key={value} type="button" onClick={() => setFormat(value)} className={`h-8 rounded-lg px-3 text-[10px] font-bold uppercase tracking-wider ${format === value ? "bg-amber-500 text-white" : "border border-amber-200 bg-white text-amber-700"}`}>{value}</button>)}</div></div>
    ...
    <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-[10px] text-amber-700">{t("bank.statement.dedupe")}</p><button type="button" onClick={() => onImport(format, content)} disabled={importing || !content.trim()} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 text-xs font-bold text-white hover:bg-amber-600 disabled:opacity-50">{importing ? <LoaderCircle className="size-3.5 animate-spin" /> : <Upload className="size-3.5" />}Importar extrato</button></div>
  </div>;
}
  ...
  return <div className={`px-4 py-3 sm:px-5 ${transaction.status === "MATCHED" ? "bg-green-50/40" : "border-l-2 border-l-amber-300 bg-white"}`}>
    <div className="flex items-start justify-between gap-4">
      ...
      <div className="flex shrink-0 items-center gap-3">
        <p className={`text-xs font-black tabular-nums ${isCredit ? "text-green-600" : "text-red-600"}`}>{isCredit ? "+" : "−"} {formatCurrency(transaction.amount, locale)}</p>
        {transaction.status === "MATCHED" ? <span className="rotate-[-6deg] rounded-md border border-green-300 px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.18em] text-green-700">{t("bank.match.matchedLabel")}</span> : null}
      </div>
    </div>
    {transaction.status === "UNMATCHED" && <div className="mt-3 flex flex-col gap-2 rounded-xl bg-slate-50 p-3 sm:flex-row sm:items-center">
      <select value={entryId} onChange={(event) => setEntryId(event.target.value)} className="h-9 flex-1 rounded-lg border border-slate-200 bg-white px-2.5 text-xs text-slate-800 outline-none focus:border-orange-300"><option value="">{t("bank.match.placeholder")}</option>{candidates.map((entry) => <option key={entry.id} value={entry.id}>{entry.description} · {formatCurrency(entry.outstandingAmount, locale)}</option>)}</select>
      <div className="flex gap-2"><button type="button" onClick={() => entryId && onMatch(entryId)} disabled={!entryId} className="h-9 rounded-lg bg-green-600 px-3 text-[11px] font-bold text-white hover:bg-green-700 disabled:opacity-50">{t("bank.actions.match")}</button><button type="button" onClick={onIgnore} className="h-9 rounded-lg border border-slate-200 bg-white px-3 text-[11px] font-bold text-slate-500 hover:bg-slate-100">{t("bank.actions.ignore")}</button></div>
    </div>}
  </div>;
}
...
function NewAccountDialog({ onClose }: { onClose: () => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : t("bank.accounts.createFailed"));
    }
  }

  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-orange-600">{t("bank.title")}</p><h2 className="mt-1 text-lg font-black text-slate-950">{t("bank.actions.create")}</h2></div><button type="button" onClick={onClose} aria-label="Fechar" className="flex size-9 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100"><XCircle className="size-4" /></button></div><form onSubmit={(event) => void submit(event)} className="mt-4 space-y-3"><label className="block text-xs font-bold text-slate-700">{t("bank.accounts.name")}<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex.: Itaú — conta PJ" className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">Tipo<select value={type} onChange={(event) => setType(event.target.value as AccountType)} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm">{(Object.keys(accountTypeLabels) as AccountType[]).map((value) => <option key={value} value={value}>{accountTypeLabels[value]}</option>)}</select></label><div className="grid grid-cols-2 gap-3"><label className="block text-xs font-bold text-slate-700">{t("bank.accounts.institution")}<input value={institution} onChange={(event) => setInstitution(event.target.value)} placeholder="Ex.: Itaú" className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label><label className="block text-xs font-bold text-slate-700">{t("bank.accounts.branch")}<input value={agency} onChange={(event) => setAgency(event.target.value)} placeholder="0001" className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label></div><label className="block text-xs font-bold text-slate-700">Número da conta<input value={accountNumber} onChange={(event) => setAccountNumber(event.target.value)} placeholder="00000-0" className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label>{type === "PIX" && <label className="block text-xs font-bold text-slate-700">{t("bank.accounts.pixKey")}<input value={pixKey} onChange={(event) => setPixKey(event.target.value)} placeholder="CNPJ, e-mail ou chave aleatória" className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label>}<label className="block text-xs font-bold text-slate-700">Saldo inicial<input type="number" step="0.01" value={openingBalance} onChange={(event) => setOpeningBalance(event.target.value)} placeholder="0,00" className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm" /></label>{error && <p className="rounded-xl bg-red-50 p-3 text-xs text-red-700">{error}</p>}<div className="flex justify-end gap-2 pt-2"><button type="button" onClick={onClose} className="h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-slate-600 hover:bg-slate-50">{t("bank.actions.cancel")}</button><button type="submit" disabled={!name.trim() || createAccount.isPending} className="inline-flex h-10 items-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-xs font-bold text-white disabled:opacity-50">{createAccount.isPending && <LoaderCircle className="size-3.5 animate-spin" />}Criar conta</button></div></form></div></div>;
}

function EmptyAccounts({ onCreate }: { onCreate: () => void }) {
  const t = useT();
  return <div className="mt-5 flex min-h-64 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-white p-8 text-center"><Landmark className="size-8 text-slate-300" /><h2 className="mt-3 text-sm font-black text-slate-900">{t("bank.accounts.empty")}</h2><p className="mt-1 max-w-sm text-xs text-slate-500">Crie caixa, conta bancária, carteira Pix ou cartão para separar o dinheiro e importar extratos.</p><button type="button" onClick={onCreate} className="mt-4 inline-flex h-10 items-center gap-2 rounded-xl bg-orange-600 px-4 text-xs font-bold text-white hover:bg-orange-700"><Plus className="size-4" />{t("bank.actions.createFirst")}</button></div>;
}
