...
export default function SaleReturnForm({ sale, onClose, onDone }: { sale: Sale; onClose: () => void; onDone: (message: string) => void }) {
  ...
  async function submit() {
    if (!reason.trim() || reason.trim().length < 3) {
      setError(t("sales.returnForm.errors.reason"));
      return;
    }
    if (!selected.length) {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : t("sales.returnForm.errors.request"));
    }
  }
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm">
      <div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-[10px] font-bold uppercase tracking-wider text-orange-600">{t("sales.returnForm.eyebrow")}</p><h2 className="mt-1 text-lg font-black text-slate-950">{t("sales.returnForm.titleOf", { code: sale.code })}</h2><p className="mt-1 text-xs text-slate-500">Os itens devolvidos voltam ao estoque com a justificativa registrada.</p></div>
          ...
          {sale.items.map((item) => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-3 p-3">
                <div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-slate-800">{item.productName}</p><p className="mt-0.5 text-[10px] text-slate-400">{t("sales.returnForm.sold", { quantity: item.quantity })} · já devolvido {item.returnedQuantity} · resta {remaining}</p></div>
        ...
        <label htmlFor="returnReason" className="mt-4 block text-xs font-bold text-slate-700">{t("sales.returnForm.reason")}</label>
        ...
        {selected.length > 0 && <p className="mt-2 rounded-xl bg-slate-50 p-3 text-xs text-slate-600">{t("sales.returnForm.selected", { count: selected.reduce((sum, entry) => sum + entry.quantity, 0) })} para devolução.</p>}
