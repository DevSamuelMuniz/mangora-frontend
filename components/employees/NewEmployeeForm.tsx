  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await createEmployee.mutateAsync({
        ...
        birthDate: data.get("birthDate") ? brazilDateTimeToIso(String(data.get("birthDate"))) : undefined,
    ...
    <form onSubmit={handleSubmit} className="mt-5 space-y-4">
      <div className="grid items-start gap-4 lg:grid-cols-[1fr_0.72fr]">
        <div className="space-y-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><SectionTitle icon={UserPlus} title={t("employees.form.dataSection")} description={t("employees.form.dataHint")} /><div className="mt-4 grid gap-4 sm:grid-cols-2">
...
const inputClassName = "h-11 w-full rounded-xl border border-slate-200 bg-white px-3.5 text-sm text-slate-950 outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-100";
...
function Toggle({ name, label }: { name: string; label: string }) { return <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2"><span className="text-[11px] font-bold text-slate-700">{label}</span><input name={name} type="checkbox" defaultChecked className="size-4 accent-orange-600" /></label>; }
