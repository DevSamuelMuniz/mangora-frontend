...
export default function EmployeeManagement() {
  ...
  async function toggleStatus() {
    ...
    try {
      ...
      setFeedback(updated.active ? "Acesso ativado." : "Acesso inativado e sessões encerradas.");
      setToToggle(null);
  ...
  async function updateRole(employee: Employee, nextRole: EmployeeRole) {
    try {
      ...
      setSelected(updated);
      setFeedback("Função e permissões atualizadas.");
    } catch (cause) { setActionError(cause instanceof Error ? cause.message : "Não foi possível alterar a função."); }
  }
  ...
  return <section>
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">Equipe e acessos</p><h1 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">Funcionários</h1><p className="mt-1 text-xs text-slate-500">Acessos, vínculos e datas importantes reunidos em um só lugar.</p></div><Link href="/funcionarios?acao=novo" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 px-4 text-sm font-bold text-white shadow-lg shadow-orange-200"><UserPlus className="size-4" />Novo funcionário</Link></div>

    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Summary label="Pessoas na equipe" value={employees.length} icon={<Users className="size-4" />} /><Summary label="Acessos ativos" value={employees.filter((e) => e.active).length} icon={<UserRoundCheck className="size-4" />} tone="green" /><Summary label="Acessos inativos" value={employees.filter((e) => !e.active).length} icon={<UserRoundX className="size-4" />} tone="slate" /><Summary label="Aniversários em 30 dias" value={upcoming.length} icon={<Cake className="size-4" />} tone="amber" /></div>

    {upcoming.length > 0 && <div className="mt-4 overflow-hidden rounded-2xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 p-4"><div className="flex items-center gap-2"><CalendarHeart className="size-4 text-orange-600" /><p className="text-xs font-black text-slate-900">Próximas celebrações</p></div><div className="mt-3 flex gap-2 overflow-x-auto pb-1">{upcoming.slice(0, 6).map(({ employee, days }) => <button key={employee.id} type="button" onClick={() => setSelected(employee)} className="shrink-0 rounded-xl border border-orange-100 bg-white px-3 py-2 text-left shadow-sm"><p className="text-[11px] font-black text-slate-800">{employee.name}</p><p className="mt-0.5 text-[9px] font-bold text-orange-600">{days === 0 ? "Hoje 🎉" : `Em ${days} dia(s)`}</p></button>)}</div></div>}
    ...
    <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="grid gap-3 md:grid-cols-3"><label className="relative"><Search className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input aria-label="Buscar funcionários" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nome, e-mail, telefone ou cargo..." className={`${inputClass} pl-10`} /></label><select aria-label="Filtrar por função" value={role} onChange={(event) => setRole(event.target.value)} className={inputClass}><option value="all">Todas as funções</option>{roles.map((item) => <option key={item} value={item}>{roleLabels[item]}</option>)}</select><select aria-label="Filtrar por status" value={status} onChange={(event) => setStatus(event.target.value)} className={inputClass}><option value="all">Todos os acessos</option><option value="true">Ativos</option><option value="false">Inativos</option></select></div></div>

    <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      {loading ? <Empty><LoaderCircle className="size-5 animate-spin text-orange-600" />Carregando equipe...</Empty> : filtered.length ? <>
        ...
        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-left"><thead className="border-b border-slate-200 bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">Funcionário</th><th className="px-4 py-3">Contato</th><th className="px-4 py-3">Função</th><th className="px-4 py-3">Datas</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-center">Ação</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((employee) => <tr key={employee.id} className="hover:bg-orange-50/30"><td className="px-5 py-3.5"><p className="text-xs font-bold text-slate-800">{employee.name}</p><p className="mt-1 text-[10px] text-slate-400">{employee.jobTitle || "Cargo não informado"}</p></td><td className="px-4 py-3.5"><p className="text-xs font-semibold text-slate-700">{employee.email}</p><p className="mt-1 text-[10px] text-slate-400">{employee.phone || "Sem telefone"}</p></td><td className="px-4 py-3.5"><RoleBadge role={employee.role} /></td><td className="px-4 py-3.5"><p className="text-[10px] text-slate-600">Admissão: {formatDate(employee.startDate)}</p><p className="mt-1 text-[10px] text-slate-400">Nascimento: {employee.birthDate ? formatDate(employee.birthDate) : "não informado"}</p></td><td className="px-4 py-3.5"><StatusBadge active={employee.active} /></td><td className="px-4 py-3.5 text-center"><button type="button" onClick={() => setSelected(employee)} className="inline-flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-[11px] font-bold text-slate-600 hover:border-orange-200 hover:bg-orange-50 hover:text-orange-700"><Pencil className="size-3.5" />Gerenciar</button></td></tr>)}</tbody></table></div>
      </> : <Empty>Nenhum funcionário corresponde aos filtros.</Empty>}
    </div>
...
function EmployeeModal({ employee, saving, onClose, onRole, onEdit, onToggle }: { employee: Employee; saving: boolean; onClose: () => void; onRole: (employee: Employee, role: EmployeeRole) => Promise<void>; onEdit: (event: FormEvent<HTMLFormElement>) => Promise<void>; onToggle: () => void }) {
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur-sm sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div role="dialog" aria-modal="true" aria-labelledby="employee-dialog-title" className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
    ...
    <form key={`${employee.id}:${employee.startDate}:${employee.birthDate ?? ""}`} onSubmit={(event) => void onEdit(event)} className="overflow-y-auto p-5 sm:p-6">
      ...
      <footer className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="h-11 rounded-xl border border-slate-200 px-5 text-xs font-bold text-slate-600">Fechar</button><button type="submit" disabled={saving} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-orange-600 px-5 text-xs font-black text-white shadow-md shadow-orange-200 disabled:opacity-60">{saving ? <LoaderCircle className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}Salvar alterações</button></footer>
    </form>
  </div></div>;
}
