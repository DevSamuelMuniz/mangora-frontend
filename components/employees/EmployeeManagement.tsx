...
import { BarChart3, Cake, CalendarHeart, CheckCircle2, LoaderCircle, Pencil, Search, ShieldCheck, UserPlus, UserRoundCheck, UserRoundX, Users, X } from "lucide-react";
import type { Employee, EmployeeRole } from "@/types/employee";
import { roleLabels, rolePermissions } from "./employee-data";
import EmployeeSalesModal from "./EmployeeSalesModal";
...
/**
 * `canManage` (dono/administrador) libera cadastro, papel e status. O gerente
 * abre a mesma tela só para acompanhar as vendas de cada funcionário — as
 * mutações continuam bloqueadas no backend pelo `RolesGuard`.
 */
export default function EmployeeManagement({ canManage = true }: { canManage?: boolean }) {
  ...
  const [salesFor, setSalesFor] = useState<Employee | null>(null);
  ...
  return <section>
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">Equipe e acessos</p><h1 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">{t("employees.title")}</h1><p className="mt-1 text-xs text-slate-500">{t("employees.subtitle")}</p></div>{canManage && <Link href="/funcionarios?acao=novo" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 px-4 text-sm font-bold text-white shadow-lg shadow-orange-200"><UserPlus className="size-4" />{t("employees.actions.create")}</Link>}</div>
    ...
    {salesFor && <EmployeeSalesModal employee={salesFor} onClose={() => setSalesFor(null)} />}
    {toToggle && <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/50 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setToToggle(null); }}><div role="alertdialog" aria-modal="true" className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl"><h2 className="text-base font-black text-slate-950">{toToggle.active ? t("employees.accessTitleDeactivate", { name: toToggle.name }) : t("employees.accessTitleActivate", { name: toToggle.name })}</h2><p className="mt-2 text-xs leading-5 text-slate-500">{toToggle.active ? t("employees.deactivateHint") : t("employees.activateHint")}</p><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setToToggle(null)} className="h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold">{t("employees.back")}</button><button type="button" disabled={saving} onClick={() => void toggleStatus()} className="h-10 rounded-xl bg-orange-600 px-4 text-xs font-bold text-white disabled:opacity-60">{t("employees.confirm")}</button></div></div></div>}
  </section>;
}
...
function EmployeeModal({ employee, saving, onClose, onRole, onEdit, onToggle }: { employee: Employee; saving: boolean; onClose: () => void; onRole: (employee: Employee, role: EmployeeRole) => Promise<void>; onEdit: (event: FormEvent<HTMLFormElement>) => Promise<void>; onToggle: () => void }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur-sm sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div role="dialog" aria-modal="true" aria-labelledby="employee-dialog-title" className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
    ...
    <form key={`${employee.id}:${employee.startDate}:${employee.birthDate ?? ""}`} onSubmit={(event) => void onEdit(event)} className="overflow-y-auto p-5 sm:p-6">
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><Field label={t("employees.roleAccess")} id="employeeRole" className="flex-1"><select id="employeeRole" disabled={saving || employee.role === "OWNER"} value={employee.role} onChange={(event) => void onRole(employee, event.target.value as EmployeeRole)} className={inputClass}>{roles.filter((item) => item !== "OWNER" || employee.role === "OWNER").map((item) => <option key={item} value={item}>{roleLabels[item]}</option>)}</select></Field>{employee.role !== "OWNER" && <button type="button" onClick={onToggle} className={`h-11 rounded-xl border px-4 text-xs font-black ${employee.active ? "border-red-200 bg-white text-red-600 hover:bg-red-50" : "border-green-200 bg-white text-green-700 hover:bg-green-50"}`}>{employee.active ? t("employees.deactivate") : t("employees.activate")}</button>}</div><ul className="mt-4 grid gap-2 sm:grid-cols-2">{rolePermissions[employee.role].map((item) => <li key={item} className="flex gap-2 text-[11px] text-slate-600"><CheckCircle2 className="mt-0.5 size-3.5 shrink-0 text-green-500" />{item}</li>)}</ul></div>
...
function EmployeeCard({ employee, canManage, onOpen, onSales }: { employee: Employee; canManage: boolean; onOpen: () => void; onSales: () => void }) { return <div className="flex w-full items-center gap-3 p-4 hover:bg-orange-50/40"><button type="button" onClick={onOpen} disabled={!canManage} className="flex min-w-0 flex-1 items-center gap-3 text-left disabled:cursor-default"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-orange-50 font-black text-orange-700">{employee.name.slice(0, 1).toUpperCase()}</span><span className="min-w-0 flex-1"><strong className="block truncate text-xs text-slate-900">{employee.name}</strong><span className="mt-1 block truncate text-[10px] text-slate-500">{employee.jobTitle || roleLabels[employee.role]} · {employee.email}</span></span></button><div className="flex shrink-0 items-center gap-2"><StatusBadge active={employee.active} /><button type="button" aria-label={`${employee.name}: vendas`} onClick={onSales} className="grid size-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:border-orange-200 hover:bg-orange-50 hover:text-orange-700"><BarChart3 className="size-3.5" /></button></div></div>; }
