...
export default function SalePreview({ cart, subtotal, discount, total, customerName, paymentMethod, customerDocument }: SalePreviewProps) {
    return (
        <div className="sticky top-4 flex flex-col overflow-hidden rounded-2xl bg-[#fff8ea] text-[#123d2b] shadow-2xl shadow-black/30">
            <div className="flex items-center justify-between border-b-2 border-dashed border-[#123d2b]/15 px-4 py-3">
                <h3 className="flex items-center gap-2 font-[family-name:var(--font-bricolage)] text-xs font-black uppercase tracking-wide">
                    <ReceiptText className="size-4 text-orange-600" /> Pré-visualização
                </h3>
                <span className="font-mono text-[10px] text-[#123d2b]/50">{cart.reduce((sum, item) => sum + item.quantity, 0)} item(ns)</span>
            </div>
                    ...
                    cart.map(({ product, quantity }) => (
                        <div key={product.id} className="flex items-center justify-between gap-2 text-xs">
                            <p className="min-w-0 truncate font-bold">
                                {product.name} <span className="font-mono text-[#123d2b]/50">× {quantity}</span>
                            </p>
                            <strong className="shrink-0 font-mono">{formatCurrency(product.price * quantity)}</strong>
                        </div>
                    ))
                ) : (
                    <p className="py-4 text-center font-mono text-[10px] text-[#123d2b]/50">Carrinho vazio.</p>
                )}
            </div>

            <div className="space-y-1 border-t-2 border-dashed border-[#123d2b]/15 px-4 py-3 font-mono text-[11px] text-[#123d2b]/70">
                ...
                <div className="flex items-end justify-between border-t border-[#123d2b]/10 pt-1.5">
                    <span className="font-[family-name:var(--font-bricolage)] text-xs font-black uppercase">Total</span>
                    <strong className="font-[family-name:var(--font-bricolage)] text-2xl font-black leading-none text-orange-600">{formatCurrency(total)}</strong>
                </div>
            </div>

            <div className="space-y-1 border-t-2 border-dashed border-[#123d2b]/15 px-4 py-3 font-mono text-[10px] text-[#123d2b]/60">
