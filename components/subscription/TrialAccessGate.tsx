    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#123d2b]/80 p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="trial-ended-title">
        <div className="relative w-full max-w-xl overflow-hidden rounded-[2rem] border-2 border-[#123d2b] bg-white p-6 shadow-[10px_10px_0_#ffb21a] sm:p-9">
          <button type="button" onClick={continueOnFree} aria-label={t("billing.trialGate.closeWarning")} className="absolute right-5 top-5 flex size-9 items-center justify-center rounded-full border border-[#123d2b]/10 text-[#315847] transition hover:bg-[#fff8ea]"><X className="size-4" /></button>
          ...
          <p className="mt-6 text-xs font-black uppercase tracking-[0.18em] text-[#ce4a0a]">{t("billing.trialGate.eyebrow")}</p>
          <h2 id="trial-ended-title" className="mt-2 max-w-md font-[family-name:var(--font-bricolage)] text-3xl font-black leading-tight text-[#123d2b] sm:text-4xl">{t("billing.trialGate.endedTitle")}</h2>
          <p className="mt-4 max-w-lg text-sm font-semibold leading-6 text-[#597064]">{t("billing.trialGate.copyFull")} {canResolveBilling ? t("billing.trialGate.upgradeHint") : t("billing.trialGate.ownerHint")}</p>
          <div className="mt-7 grid gap-3 sm:grid-cols-[1fr_auto]">{canResolveBilling && <Link href="/assinatura" className="group flex h-12 items-center justify-center gap-2 rounded-xl bg-[#ff6b1a] px-5 text-sm font-extrabold text-white shadow-[0_4px_0_#c9460b] transition hover:-translate-y-0.5">{t("billing.trialGate.comparePlans")}<ArrowRight className="size-4 transition group-hover:translate-x-1" /></Link>}<button type="button" onClick={continueOnFree} className="flex h-12 items-center justify-center rounded-xl border-2 border-[#123d2b]/15 px-5 text-xs font-extrabold text-[#315847] hover:bg-[#fff8ea]">Continuar no Free</button></div>
          <div className="mt-6 flex items-center gap-2 border-t border-[#123d2b]/10 pt-5 text-xs font-semibold text-[#597064]"><ShieldCheck className="size-4 text-[#147a45]" />{t("billing.trialGate.nothingDeleted")}</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="border-b border-[#ffb21a]/30 bg-[#fff0dd] px-4 py-2.5 text-[#123d2b] sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-2 text-xs">
          <p className="flex items-center gap-2 font-bold"><CalendarClock className="size-4 text-[#ce4a0a]" /><strong>{t(company.trialDaysRemaining === 1 ? "billing.trialGate.dayRemaining" : "billing.trialGate.daysRemainingBanner", { count: company.trialDaysRemaining })}</strong><span className="hidden font-medium text-[#597064] sm:inline">{t("billing.trialGate.exploreHint")}</span></p>
          {canResolveBilling && <Link href="/assinatura" className="font-black text-[#a93a05] underline decoration-2 underline-offset-4">{t("billing.trialGate.seePlans")}</Link>}
        </div>
      </div>
    );
  }
  ...
  if (company.subscriptionPlan === "FREE" && !billingRoute) {
    return (
      <div className="border-b border-[#147a45]/20 bg-[#edf8f1] px-4 py-2.5 text-[#123d2b] sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-2 text-xs"><p className="flex items-center gap-2 font-bold"><Gift className="size-4 text-[#147a45]" /><strong>{t("billing.trialGate.freeActive")}</strong><span className="hidden font-medium text-[#597064] sm:inline">{t("billing.trialGate.freeHint")}</span></p>{canResolveBilling && <Link href="/assinatura" className="font-black text-[#147a45] underline decoration-2 underline-offset-4">Conhecer recursos pagos</Link>}</div>
      </div>
    );
  }
