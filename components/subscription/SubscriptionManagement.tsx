    ...
    try {
      ...
      if (selected.id === "enterprise") {
        ...
        const checkout = await checkoutMutation.mutateAsync({
          targetPlan: selected.id.toUpperCase(), billingType, nextDueDate, couponCode: couponCode.trim() || undefined,
        });
        setMessage(checkout.discount > 0 ? `Cupom aplicado: primeira mensalidade por ${formatCurrency(checkout.firstCharge)}. As próximas serão de ${formatCurrency(checkout.recurringPrice)}.` : "Cobrança criada no Asaas. O plano será ativado assim que o pagamento for confirmado.");
      }
      setSelected(null);
...
{overview && <ContractAndHistory overview={overview} />}
    ...
    {selected && <div className="rounded-2xl border border-orange-200 bg-orange-50 p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold text-orange-900">{selected.id === "enterprise" ? "Solicitar contato sobre Enterprise?" : `Contratar ${selected.name} pelo Asaas`}</p><p className="mt-1 text-[10px] text-orange-600">{selected.id === "enterprise" ? "Nossa equipe comercial entrará em contato." : overview.provider.configured ? "O plano atual só será alterado após a confirmação do pagamento. Cupons reduzem somente a primeira mensalidade." : "A integração ainda precisa ser configurada no backend."}</p>{selected.id !== "enterprise" && <div className="mt-3 flex flex-wrap gap-3"><label className="text-[10px] font-bold text-orange-900">Pagamento<select value={billingType} onChange={(event) => setBillingType(event.target.value as "PIX" | "BOLETO")} className="mt-1 block h-10 rounded-xl border border-orange-200 bg-white px-3 text-xs"><option value="PIX">PIX</option><option value="BOLETO">Boleto</option></select></label><label className="text-[10px] font-bold text-orange-900">Primeiro vencimento<input type="date" min={defaultDueDate()} value={nextDueDate} onChange={(event) => setNextDueDate(event.target.value)} className="mt-1 block h-10 rounded-xl border border-orange-200 bg-white px-3 text-xs" /></label><label className="text-[10px] font-bold text-orange-900">Cupom (opcional)<input value={couponCode} onChange={(event) => setCouponCode(event.target.value.toUpperCase())} maxLength={32} placeholder="EX.: BEMVINDO20" className="mt-1 block h-10 rounded-xl border border-orange-200 bg-white px-3 text-xs uppercase" /></label></div>}</div><div className="flex gap-2"><button onClick={() => setSelected(null)} className="h-10 rounded-xl border border-orange-200 bg-white px-4 text-xs font-bold text-orange-700">Voltar</button><button disabled={saving || (selected.id !== "enterprise" && !overview.provider.configured)} onClick={() => void requestChange()} className="h-10 rounded-xl bg-orange-600 px-4 text-xs font-bold text-white disabled:opacity-60">{saving ? "Processando..." : selected.id === "enterprise" ? "Solicitar contato" : "Criar cobrança"}</button></div></div></div>}
