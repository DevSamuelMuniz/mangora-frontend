...
export default function SubscriptionManagement() {
  ...
  async function requestChange() {
    ...
    try {
      ...
      if (selected.id === "enterprise") {
        ...
        else setMessage((checkout.discount ?? 0) > 0 && checkout.firstCharge != null && checkout.recurringPrice != null ? `Cupom aplicado: primeira mensalidade por ${formatCurrency(checkout.firstCharge)}. As próximas serão de ${formatCurrency(checkout.recurringPrice)}.` : `Cobrança criada no ${overview.provider.name}. O plano será ativado assim que o pagamento for confirmado.`);
      }
      setSelected(null);
  ...
  async function reactivateNow() {
    ...
    } catch (cause) { setActionError(cause instanceof Error ? cause.message : "Não foi possível reativar a assinatura."); }
  }
...
function PlanCard({ plan, displayPrice, current, onSelect }: { plan: SubscriptionPlan; displayPrice?: string; current: boolean; onSelect: (plan: SubscriptionPlan) => void }) { const freeUnavailable = plan.id === "free" && !current; return <article className={`flex flex-col rounded-2xl border bg-white p-4 shadow-sm ${plan.highlighted ? "border-orange-300 ring-2 ring-orange-100" : "border-slate-200"}`}><div className="flex justify-between"><div><h3 className="text-sm font-black text-slate-950">{plan.name}</h3><p className="mt-1 min-h-8 text-[10px] leading-4 text-slate-500">{plan.description}</p></div><Building2 className="size-4 text-orange-500" /></div><p className="mt-4 border-b border-slate-100 pb-4 text-lg font-black text-slate-950">{plan.price === null ? "Sob consulta" : plan.price === 0 ? "Grátis" : displayPrice ?? formatCurrency(plan.price)}</p><ul className="my-4 flex-1 space-y-2.5">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-[10px] leading-4 text-slate-600"><Check className="size-3.5 shrink-0 text-green-500" />{feature}</li>)}</ul><button disabled={current || freeUnavailable} onClick={() => onSelect(plan)} className={`flex h-10 items-center justify-center gap-2 rounded-xl text-xs font-bold ${current ? "bg-orange-50 text-orange-700" : freeUnavailable ? "bg-slate-50 text-slate-400" : "border border-slate-200 text-slate-700 hover:bg-orange-50"}`}>{current ? <><CheckCircle2 className="size-3.5" />Plano atual</> : freeUnavailable ? "Incluído por padrão" : <>{plan.id === "enterprise" ? "Solicitar contato" : `Solicitar ${plan.name}`}<ArrowRight className="size-3.5" /></>}</button></article>; }
