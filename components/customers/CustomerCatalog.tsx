    ...
    <>
      <section>
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          ...
          <Link href="/clientes/novo" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-sm font-bold text-white shadow-lg shadow-orange-200 transition hover:-translate-y-0.5 hover:shadow-xl"><UserPlus className="size-4" />Novo cliente</Link>
        </div>
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          {loading ? (
            ...
            <div className="flex min-h-80 flex-col items-center justify-center px-6 py-12 text-center"><div className="flex size-12 items-center justify-center rounded-2xl bg-orange-50 text-orange-600"><UserRoundX className="size-5" /></div><h2 className="mt-4 text-sm font-bold text-slate-900">Nenhum cliente encontrado</h2><p className="mt-1 max-w-sm text-xs leading-5 text-slate-500">{customers.length === 0 ? "Cadastre o primeiro cliente para começar sua base." : "Ajuste os filtros para visualizar sua base."}</p>{customers.length === 0 ? <Link href="/clientes/novo" className="mt-4 inline-flex h-10 items-center rounded-xl bg-orange-600 px-4 text-xs font-bold text-white">Cadastrar cliente</Link> : <button type="button" onClick={clearFilters} className="mt-4 h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-orange-600 transition hover:bg-orange-50">Limpar filtros</button>}</div>
          )}
        </div>
      </section>
