                  ...
                  <tbody className="divide-y divide-slate-100">
                    {visibleCustomers.map((customer) => (
                      <tr key={customer.id} className="transition hover:bg-slate-50">
                        <td className="px-5 py-3.5"><div className="flex items-center gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-orange-100 to-yellow-50 text-xs font-black text-orange-700">{getInitials(customer.name)}</div><div className="min-w-0"><p className="max-w-64 truncate text-xs font-bold text-slate-800">{customer.tradeName || customer.name}</p><p className="mt-1 text-[10px] text-slate-400">{formatDocument(customer.document, customer.type)} · {customer.type === "INDIVIDUAL" ? t("customers.types.INDIVIDUAL") : t("customers.types.COMPANY")}</p></div></div></td>
...
function CustomerDetails({ customer, onClose }: { customer: Customer; onClose: () => void }) {
  ...
  const details = [["Documento", formatDocument(customer.document, customer.type)], ["Tipo", customer.type === "INDIVIDUAL" ? t("customers.types.INDIVIDUAL") : t("customers.types.COMPANY")], ["E-mail", customer.email], ["Telefone", formatPhone(customer.phone)], ["Endereço", location], ["CEP", customer.postalCode ? customer.postalCode.replace(/^(\d{5})(\d{3})$/, "$1-$2") : t("customers.details.notInformed")], ["Total comprado", formatCurrency(customer.totalSpent, locale)], ["Última compra", customer.lastPurchaseAt ? formatDate(new Date(customer.lastPurchaseAt)) : "Nunca"]];
  return <div onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-orange-600">{t("customers.details.title")}</p><h2 className="mt-1 text-lg font-black text-slate-950">{customer.tradeName || customer.name}</h2>{customer.tradeName && <p className="mt-1 text-xs text-slate-500">{customer.name}</p>}</div><button type="button" onClick={onClose} aria-label="Fechar detalhes" className="flex size-9 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100"><X className="size-4" /></button></div><dl className="mt-5 grid gap-3 sm:grid-cols-2">{details.map(([label, value]) => <div key={label} className="min-w-0 rounded-xl bg-slate-50 p-3"><dt className="text-[10px] font-semibold text-slate-400">{label}</dt><dd className="mt-1 break-words text-xs font-bold text-slate-800">{value}</dd></div>)}</dl>{customer.notes && <p className="mt-3 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-600">{customer.notes}</p>}<div className="mt-5 flex justify-end"><Link href={`/clientes?acao=editar&id=${customer.id}`} className="inline-flex h-10 items-center gap-2 rounded-xl bg-orange-600 px-4 text-xs font-bold text-white"><Pencil className="size-3.5" />{t("customers.actions.edit")}</Link></div></div></div>;
}
