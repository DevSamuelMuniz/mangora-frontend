...
import type { FiscalReadiness, FiscalSettingsResponse } from "@/types/fiscal";

export default function FiscalSettingsPanel() {
  const [data, setData] = useState<FiscalSettingsResponse | null>(null);
  ...
  useEffect(() => {
    let mounted = true;
    Promise.all([apiRequest<FiscalSettingsResponse>("/fiscal/settings"), apiRequest<FiscalReadiness>("/fiscal/readiness")])
      .then(([settings, status]) => { if (mounted) { setData(settings); setReadiness(status); } })
      .catch((cause: unknown) => { if (mounted) setError(cause instanceof Error ? cause.message : "Não foi possível carregar a configuração fiscal."); })
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiRequest<FiscalSettingsResponse>("/fiscal/settings", { method: "PATCH", body: JSON.stringify({
      ...
      setData(result); setReadiness(await apiRequest<FiscalReadiness>("/fiscal/readiness")); setMessage("Configuração fiscal salva para esta loja.");
  ...
  async function connectProvider() {
    ...
    try {
      const result = await apiRequest<FiscalSettingsResponse>("/fiscal/provider/connect", { method: "POST", body: JSON.stringify({ token: providerToken.trim() }) });
      setData(result); setProviderToken(""); setReadiness(await apiRequest<FiscalReadiness>("/fiscal/readiness"));
      setMessage(`Focus NFe conectada em ${result.settings.environment === "HOMOLOGATION" ? "homologação" : "produção"}.`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível conectar a Focus NFe."); }
  ...
  if (!data) return <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs font-bold text-red-800">{error || "Configuração indisponível."}</div>;
  ...
  return <section className="mx-auto max-w-6xl space-y-5"><div><p className="text-[11px] font-black uppercase tracking-[0.14em] text-orange-600">Fiscal por unidade</p><h1 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">Configuração fiscal</h1><p className="mt-1 text-xs text-slate-500">Dados exclusivos de {company.tradeName}. Confirme a tributação com o contador.</p></div>
    ...
    <form key={`${settings.environment}-${settings.nfeSeries}-${settings.nfceSeries}`} onSubmit={(event) => void submit(event)} className="space-y-4">
      ...
      {error && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-bold text-red-800">{error}</div>}{message && <div role="status" className="rounded-xl border border-green-200 bg-green-50 p-3 text-xs font-bold text-green-800">{message}</div>}<div className="flex justify-end"><button type="submit" disabled={saving} className="flex h-11 items-center gap-2 rounded-xl bg-orange-600 px-5 text-xs font-black text-white shadow-lg shadow-orange-200 disabled:opacity-50">{saving ? <LoaderCircle className="size-4 animate-spin" /> : <Save className="size-4" />}Salvar configuração fiscal</button></div>
    </form>
  </section>;
}
