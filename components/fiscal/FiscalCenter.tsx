...
import type { FiscalDocument, FiscalDocumentStatus, FiscalDocumentType, FiscalReadiness } from "@/types/fiscal";
...
export default function FiscalCenter() {
  ...
  useEffect(() => {
    let mounted = true;
    Promise.all([apiRequest<FiscalDocument[]>("/fiscal/documents"), apiRequest<Sale[]>("/sales"), apiRequest<FiscalReadiness>("/fiscal/readiness")])
      .then(([documentData, saleData, status]) => { if (mounted) { setDocuments(documentData); setSales(saleData); setReadiness(status); const first = saleData.find((sale) => sale.status === "COMPLETED"); if (first) setSaleId(first.id); } })
      .catch((cause: unknown) => { if (mounted) setError(cause instanceof Error ? cause.message : "Não foi possível carregar a central fiscal."); })
  ...
  async function prepare() {
    ...
    try {
      const document = await apiRequest<FiscalDocument>("/fiscal/documents/prepare", { method: "POST", body: JSON.stringify({ saleId, type }) });
      setDocuments((current) => [document, ...current]); setMessage(`${type === "NFE" ? "NF-e" : "NFC-e"} preparada. Ela ainda não foi transmitida à SEFAZ.`);
      const next = eligibleSales.find((sale) => sale.id !== saleId); setSaleId(next?.id ?? "");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível preparar o documento."); }
  ...
  async function fiscalAction(document: FiscalDocument, action: "issue" | "consult" | "cancel") {
    ...
    if (action === "cancel") {
      const reason = window.prompt("Informe a justificativa do cancelamento (15 a 255 caracteres):")?.trim();
      if (!reason) return;
      body = JSON.stringify({ reason });
    }
    setActingId(document.id); setError(""); setMessage("");
    try {
      const updated = await apiRequest<FiscalDocument>(`/fiscal/documents/${document.id}/${action}`, { method: "POST", body });
