...
export default function OrderCatalog() {
  ...
  return <>
    <section>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">Comercial</p><h1 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">Pedidos</h1><p className="mt-1 text-xs text-slate-500">Organize solicitações, reservas, retirada e entrega.</p></div><Link href="/pedidos/novo" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 px-4 text-sm font-bold text-white"><ShoppingBag className="size-4" />Novo pedido</Link></div>
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><SummaryCard icon={ShoppingBag} label="Pedidos hoje" value={String(today.length)} iconClassName="bg-orange-50 text-orange-600" /><SummaryCard icon={Clock3} label="Em andamento" value={String(active.length)} iconClassName="bg-amber-50 text-amber-600" /><SummaryCard icon={Truck} label="Aguardando entrega" value={String(delivery.length)} iconClassName="bg-yellow-50 text-yellow-600" /><SummaryCard icon={CircleDollarSign} label="Valor hoje" value={currencyFormatter.format(today.reduce((sum, order) => sum + order.total, 0))} iconClassName="bg-green-50 text-green-600" /></div>
