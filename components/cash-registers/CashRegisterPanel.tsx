...
export default function CashRegisterPanel() {
  ...
  return <section>
    ...
    {!register ? <div className="mt-5 grid items-start gap-4 lg:grid-cols-[1fr_0.75fr]"><form onSubmit={openRegister} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-3 border-b border-slate-100 pb-4"><div className="flex size-10 items-center justify-center rounded-xl bg-orange-50 text-orange-600"><Banknote className="size-4" /></div><div><h2 className="text-sm font-bold text-slate-950">{t("operations.cash.open")}</h2><p className="text-[10px] text-slate-400">Informe o dinheiro disponível no início do turno.</p></div></div><label htmlFor="openingAmount" className="mt-4 block text-xs font-bold text-slate-700">Saldo inicial</label><div className="relative mt-1.5"><span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">R$</span><input id="openingAmount" name="openingAmount" type="number" required min="0" step="0.01" defaultValue="0" className={`${inputClass} pl-10`} /></div><label htmlFor="openingNotes" className="mt-4 block text-xs font-bold text-slate-700">Observação (opcional)</label><textarea id="openingNotes" name="notes" rows={3} maxLength={500} className={textareaClass} /><button disabled={busy} className="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 text-sm font-bold text-white disabled:opacity-60">{busy ? <LoaderCircle className="size-4 animate-spin" /> : <PlusCircle className="size-4" />}Abrir caixa</button></form><div className="rounded-2xl border border-orange-100 bg-orange-50 p-5"><LockKeyhole className="size-5 text-orange-600" /><h2 className="mt-3 text-sm font-black text-orange-950">{t("operations.cash.closed")}</h2><p className="mt-2 text-xs leading-5 text-orange-700">Vendas em dinheiro e recebimentos de pedidos exigem um caixa aberto. Pagamentos eletrônicos continuam disponíveis normalmente.</p></div></div> : <>
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric locale={locale} icon={WalletCards} label="Saldo esperado" value={register.expectedAmount} tone="orange" /><Metric locale={locale} icon={Banknote} label="Vendas em dinheiro" value={totals.sales} tone="green" /><Metric locale={locale} icon={ArrowDownToLine} label="Suprimentos" value={totals.supplies} tone="yellow" /><Metric locale={locale} icon={ArrowUpFromLine} label="Saídas e estornos" value={totals.withdrawals} tone="red" /></div>
