...
export default function CashRegisterPanel() {
  ...
  async function openRegister(event: FormEvent<HTMLFormElement>) {
    ...
    await execute(async () => { await openMutation.mutateAsync({ openingAmount: Number(data.get("openingAmount")), notes: String(data.get("notes") ?? "") || undefined }); await refresh(); setFeedback("Caixa aberto com sucesso."); });
  }
  ...
  async function execute(operation: () => Promise<void>) {
    ...
    catch (cause) { setActionError(cause instanceof Error ? cause.message : "Não foi possível concluir a operação."); }
  }

  if (loading) return <div className="flex min-h-72 items-center justify-center gap-2 text-sm text-slate-500"><LoaderCircle className="size-4 animate-spin text-orange-600" />Carregando caixa...</div>;

  return <section>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-orange-600">Operação</p><h1 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">Controle de caixa</h1><p className="mt-1 text-xs text-slate-500">Abertura, movimentações em dinheiro e conferência do turno.</p></div>{register && <span className="flex items-center gap-2 rounded-full bg-green-50 px-3 py-2 text-[10px] font-bold text-green-700"><span className="size-2 rounded-full bg-green-500" />Caixa aberto</span>}</div>
    ...
    {!register ? <div className="mt-5 grid items-start gap-4 lg:grid-cols-[1fr_0.75fr]"><form onSubmit={openRegister} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-3 border-b border-slate-100 pb-4"><div className="flex size-10 items-center justify-center rounded-xl bg-orange-50 text-orange-600"><Banknote className="size-4" /></div><div><h2 className="text-sm font-bold text-slate-950">Abrir caixa</h2><p className="text-[10px] text-slate-400">Informe o dinheiro disponível no início do turno.</p></div></div><label htmlFor="openingAmount" className="mt-4 block text-xs font-bold text-slate-700">Saldo inicial</label><div className="relative mt-1.5"><span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">R$</span><input id="openingAmount" name="openingAmount" type="number" required min="0" step="0.01" defaultValue="0" className={`${inputClass} pl-10`} /></div><label htmlFor="openingNotes" className="mt-4 block text-xs font-bold text-slate-700">Observação (opcional)</label><textarea id="openingNotes" name="notes" rows={3} maxLength={500} className={textareaClass} /><button disabled={busy} className="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-600 to-amber-600 text-sm font-bold text-white disabled:opacity-60">{busy ? <LoaderCircle className="size-4 animate-spin" /> : <PlusCircle className="size-4" />}Abrir caixa</button></form><div className="rounded-2xl border border-orange-100 bg-orange-50 p-5"><LockKeyhole className="size-5 text-orange-600" /><h2 className="mt-3 text-sm font-black text-orange-950">Caixa fechado</h2><p className="mt-2 text-xs leading-5 text-orange-700">Vendas em dinheiro e recebimentos de pedidos exigem um caixa aberto. Pagamentos eletrônicos continuam disponíveis normalmente.</p></div></div> : <>
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={WalletCards} label="Saldo esperado" value={register.expectedAmount} tone="orange" /><Metric icon={Banknote} label="Vendas em dinheiro" value={totals.sales} tone="green" /><Metric icon={ArrowDownToLine} label="Suprimentos" value={totals.supplies} tone="yellow" /><Metric icon={ArrowUpFromLine} label="Saídas e estornos" value={totals.withdrawals} tone="red" /></div>
...
function Metric({ icon: Icon, label, value, tone }: { icon: typeof Banknote; label: string; value: number; tone: "orange" | "green" | "yellow" | "red" }) { const colors = { orange: "bg-orange-50 text-orange-600", green: "bg-green-50 text-green-600", yellow: "bg-yellow-50 text-yellow-600", red: "bg-red-50 text-red-600" }; return <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className={`flex size-9 items-center justify-center rounded-xl ${colors[tone]}`}><Icon className="size-4" /></div><p className="mt-3 text-[10px] font-bold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-1 text-xl font-black text-slate-950">{formatCurrency(value)}</p></div>; }
