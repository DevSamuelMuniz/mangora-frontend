...
type NotificationStyle = { icon: LucideIcon; iconClass: string; href: string; label: string };
const styles: Record<string, NotificationStyle> = {
  LOW_STOCK: { icon: Boxes, iconClass: "bg-amber-50 text-amber-700", href: "/estoque", label: "Estoque" },
  OVERDUE_ACCOUNT: { icon: CircleDollarSign, iconClass: "bg-red-50 text-red-600", href: "/financeiro", label: "Financeiro" },
  SECURITY: { icon: ShieldAlert, iconClass: "bg-red-50 text-red-600", href: "/configuracao", label: "Segurança" },
  SUMMARY: { icon: Sparkles, iconClass: "bg-green-50 text-green-700", href: "/relatorios", label: "Resumo" },
  INFO: { icon: Bell, iconClass: "bg-orange-50 text-orange-700", href: "/dashboard", label: "Informação" },
};
...
export default function NotificationCenter({ onOpen }: { onOpen?: () => void }) {
  ...
  return <div ref={rootRef} className="relative">
    ...
    {open && <div role="dialog" aria-modal="false" aria-label="Central de notificações" className="fixed left-3 right-3 top-[4.5rem] z-50 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-950/20 sm:absolute sm:left-auto sm:right-0 sm:top-auto sm:mt-2 sm:w-[400px]">
      <header className="bg-[#173d2b] px-5 pb-4 pt-5 text-white">
        <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-xl bg-white/10 text-orange-300"><Bell className="size-4" /></span><h2 className="text-base font-black">Notificações</h2></div><p className="mt-2 text-[11px] text-green-100/70">{unread ? `${unread} ${unread === 1 ? "aviso precisa" : "avisos precisam"} da sua atenção` : "Tudo acompanhado por aqui"}</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Fechar notificações" className="flex size-8 items-center justify-center rounded-xl text-white/60 hover:bg-white/10 hover:text-white"><X className="size-4" /></button></div>
        <div className="mt-4 flex items-center justify-between gap-3"><div className="flex rounded-xl bg-black/15 p-1"><FilterButton active={filter === "all"} onClick={() => setFilter("all")}>Todas <span>{notifications.length}</span></FilterButton><FilterButton active={filter === "unread"} onClick={() => setFilter("unread")}>Não lidas <span>{unread}</span></FilterButton></div><button type="button" disabled={!unread || markAll.isPending} onClick={() => markAll.mutate()} className="flex items-center gap-1.5 text-[10px] font-bold text-orange-300 hover:text-orange-200 disabled:cursor-not-allowed disabled:text-white/30">{markAll.isPending ? <LoaderCircle className="size-3.5 animate-spin" /> : <CheckCheck className="size-3.5" />}Marcar todas</button></div>
      </header>

      <div className="max-h-[min(440px,calc(100vh-12rem))] overflow-y-auto overscroll-contain">
        {isLoading ? <Empty icon={LoaderCircle} spin text="Carregando seus avisos..." /> : visible.length ? <div className="divide-y divide-slate-100">{visible.map((notification) => <NotificationRow key={notification.id} notification={notification} busy={markOne.isPending} onOpen={() => void openNotification(notification)} />)}</div> : <Empty icon={filter === "unread" ? CheckCheck : Inbox} text={filter === "unread" ? "Você não tem avisos pendentes." : "Nenhuma notificação por enquanto."} />}
      </div>
      <footer className="flex items-center justify-between border-t border-slate-100 bg-slate-50 px-5 py-3"><p className="text-[9px] font-semibold text-slate-400">{isFetching ? "Atualizando…" : "Atualizado automaticamente"}</p><button type="button" onClick={() => { setOpen(false); router.push("/configuracoes?secao=notifications"); }} className="text-[10px] font-black text-orange-700 hover:text-orange-900">Configurar alertas</button></footer>
    </div>}
  </div>;
}
...
function relativeTime(value: string) { const seconds = Math.max(0, Math.floor((Date.now() - new Date(value).getTime()) / 1000)); if (seconds < 60) return "agora"; const minutes = Math.floor(seconds / 60); if (minutes < 60) return `há ${minutes} min`; const hours = Math.floor(minutes / 60); if (hours < 24) return `há ${hours} h`; const days = Math.floor(hours / 24); return days === 1 ? "ontem" : `há ${days} dias`; }
