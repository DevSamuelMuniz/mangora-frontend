...
function getStatusStyle(status: SaleStatus) {
  return status === "COMPLETED"
    ? { className: "bg-green-50 text-green-600", icon: CheckCircle2 }
        ...
        {sales.map((sale) => {
          ...
          return (
            <div key={sale.id} className="relative flex items-center gap-3 px-4 py-3.5 transition hover:bg-slate-50 sm:px-5">
              <div className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-orange-50 text-[10px] font-black text-orange-700">{sale.customerName.split(" ").slice(0, 2).map((name) => name[0]).join("")}</div>
              ...
              <button type="button" onClick={() => setOpenSale((current) => current === sale.id ? null : sale.id)} aria-label={t("dashboard.saleOptionsAria", { code: sale.code })} aria-expanded={selected} className="flex size-8 shrink-0 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><MoreHorizontal className="size-4" /></button>
