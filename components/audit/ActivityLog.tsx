...
export default function ActivityLog() {
  ...
  return <section>
    ...
    <div className="mt-4 grid gap-3 rounded-2xl border bg-white p-4 shadow-sm sm:grid-cols-[1fr_240px]"><label className="relative"><Search className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input aria-label={t("modules.audit.searchAria")} value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("modules.audit.searchPlaceholder")} className="h-11 w-full rounded-xl border bg-white pl-10 pr-3 text-sm outline-none focus:border-orange-400" /></label><select aria-label={t("modules.audit.filterAria")} value={action} onChange={(event) => setAction(event.target.value)} className="h-11 rounded-xl border bg-white px-3 text-xs font-bold"><option value="all">{t("modules.audit.allActions")}</option>{actions.map((value) => <option key={value} value={value}>{translateLabel(value, t)}</option>)}</select></div>
