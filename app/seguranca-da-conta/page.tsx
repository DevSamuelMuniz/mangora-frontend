...
export default function AccountSecurityPage() {
  ...
  {!status.mfaEnabled && !setup && status.nextStep !== "email" && <form onSubmit={begin} className="mt-7"><Step icon={KeyRound} title="Ative o autenticador (opcional)" text="Confirme sua senha para gerar uma chave no Google Authenticator, Microsoft Authenticator ou aplicativo compatível." /><input name="password" type="password" required autoComplete="current-password" placeholder="Sua senha atual" className="input mt-5" /><button disabled={busy || !status.configured} className="button mt-3">Gerar chave do autenticador</button><Link href="/dashboard" className="mt-4 block text-center text-xs font-bold text-[#597064]">Continuar sem ativar</Link></form>}
  ...
  {status.nextStep === "mfa" && <form onSubmit={confirm} className="mt-7"><Step icon={KeyRound} title="Código de segurança" text="Digite o código atual do autenticador ou um código de recuperação." /><CodeInput /><TrustDeviceOption /><button disabled={busy} className="button mt-3">Verificar e continuar</button></form>}
...
function TrustDeviceOption() { return <label className="mt-4 flex cursor-pointer items-start gap-2.5 rounded-xl border border-[#123d2b]/10 bg-[#fff8ea] p-3 text-xs font-bold text-[#315847]"><input type="checkbox" name="trustDevice" defaultChecked className="mt-0.5 size-4 shrink-0 accent-[#147a45]" /><span>Confiar neste dispositivo por 30 dias<span className="mt-1 block text-[10px] font-medium leading-4 text-[#597064]">Use somente em computador ou celular pessoal. Você não precisará informar o código em cada login.</span></span></label>; }
